module mx68k_core_rtr_tb;
    ...
    mx_mem_size_t stack_read_size [0:1];
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (dmem_bus.req_valid && dmem_bus.req_ready &&
                (dmem_bus.req.command == MX_MEM_READ) &&
            ...
            if (cycles > 2000)
                $fatal(1, "MX68K RTR test timed out");
        end
    end
    ...
    initial begin
        ...
        assert (stack_read_size[0] == MX_SIZE_WORD);
        assert (stack_read_address[1] == 32'h0000_0202);
        assert (stack_read_size[1] == MX_SIZE_LONG);
