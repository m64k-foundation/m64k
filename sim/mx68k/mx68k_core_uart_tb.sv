module mx68k_core_uart_tb;
    ...
    mx68k_mem_if vectors_bus(.clk(clk), .rst_n(rst_n));
    mx68k_mem_if uart_bus(.clk(clk), .rst_n(rst_n));
    ...
    mx68k_ram #(.MEM_BYTES(256)) vector_ram (
        .clk, .rst_n, .mem(vectors_bus)
    );

    mx68k_uart_16550_model uart (
        .clk, .rst_n,
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (cycles > 3000)
                $fatal(1, "MX68K native UART test timed out");
        end
    end
    ...
    initial begin
        ...
        $display("PASS: native MX68K emitted '%c' through fabric UART", captured_tx);
        $finish;
    end
endmodule
