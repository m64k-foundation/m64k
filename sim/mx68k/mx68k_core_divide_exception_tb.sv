module mx68k_core_divide_exception_tb;
    ...
    task automatic run_nonzero_case(
        ...
        begin
            ...
            assert (retire_instruction_id ==
                    (signed_operation ? MX_INSN_DIVS : MX_INSN_DIVU));
            assert (debug_data_registers[0*32 +: 32] ==
                    (expected_overflow ? dividend : expected_result));
            assert (debug_sr[MX_SR_X] == 1'b1);
            assert (debug_sr[MX_SR_V] == expected_overflow);
            assert (debug_sr[MX_SR_C] == 1'b0);
            if (!expected_overflow) begin
                assert (debug_sr[MX_SR_N] == expected_n);
                assert (debug_sr[MX_SR_Z] == expected_z);
            end
    ...
    task automatic run_zero_case(input logic signed_operation);
        ...
        begin
            ...
            set_data_long(0, 32'h0000_0300);
            set_data_long(4, 32'h0000_0100);
            set_data_long(MX_VECTOR_ZERO_DIVIDE * 4, 32'h0000_0180);
