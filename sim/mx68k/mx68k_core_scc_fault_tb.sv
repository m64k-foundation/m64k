module mx68k_core_scc_fault_tb;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            cycles <= cycles + 1;
            if (cycles > 3000)
                $fatal(1, "Scc RMW fault test timed out");
        end
    end
    ...
    task automatic run_fault_case(input logic condition_true,
        ...
        begin
            ...
            opcode = condition_true ? 16'h50d8 : 16'h51d8; // ST/SF (A0)+
            ...
            // Neither failed stage may commit the candidate byte,
    ...
    initial begin
        rst_n = 1'b0;
        cycles = 0;
        for (int condition_index = 0; condition_index < 2;
             condition_index++) begin
            run_fault_case(!condition_index[0], 1'b0);
            run_fault_case(!condition_index[0], 1'b1);
        end

        $display("PASS: Scc true/false read/write faults suppress all RMW commit");
        $finish;
    end
endmodule
