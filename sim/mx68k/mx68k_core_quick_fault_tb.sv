module mx68k_core_quick_fault_tb;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            cycles <= cycles + 1;
            if (cycles > 3000)
                $fatal(1, "ADDQ/SUBQ RMW fault test timed out");
        end
    end
    ...
    task automatic run_fault_case(input logic subtract,
        ...
        begin
            ...
            opcode = subtract ? 16'h5318 : 16'h5218; // #1,(A0)+
    ...
    initial begin
        ...
        for (int operation_index = 0; operation_index < 2;
            ...
            run_fault_case(operation_index[0], 1'b0);
            run_fault_case(operation_index[0], 1'b1);
        end

        $display("PASS: ADDQ/SUBQ read/write faults suppress all RMW commit");
        $finish;
    end
endmodule
