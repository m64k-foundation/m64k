module mx68k_core_sr_tb;
    ...
    mx_mem_command_t sr_target_commands [0:1];
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (cycles > 2000)
                $fatal(1, "MX68K SR test timed out");
        end
    end
    ...
    initial begin
        ...
        assert ({data_ram.storage[10'h202], data_ram.storage[10'h203]} ==
                16'h2009);
        assert (sr_target_accesses == 2);
        assert (sr_target_commands[0] == MX_MEM_READ);
        assert (sr_target_commands[1] == MX_MEM_WRITE);
