module mx68k_arch_pkg_tb;
    import mx68k_arch_pkg::*;

    mx_alu_result_t alu_value;
    ...
    initial begin
        assert (MX_SR_M00_DEFINED_MASK == 16'ha71f);
        assert (MX_SR_M20_DEFINED_MASK == 16'hf71f);
        assert (MX_SR_CCR_MASK == 16'h001f);
        assert (MX_SR_TRACE_MASK == 16'hc000);
        assert (MX_SR_SUPERVISOR_MASK == 16'h3000);
        assert (MX_SR_INTERRUPT_MASK == 16'h0700);
        assert (MX_SR_T1 == 15 && MX_SR_T0 == 14);
        assert (MX_SR_S == 13 && MX_SR_M == 12);
        assert (MX_SR_I2 == 10 && MX_SR_I1 == 9 && MX_SR_I0 == 8);
        assert (MX_SR_X == 4 && MX_SR_N == 3 && MX_SR_Z == 2);
        assert (MX_SR_V == 1 && MX_SR_C == 0);
        assert (MX_VECTOR_RESET_SSP == 8'd0);
        assert (MX_VECTOR_RESET_PC == 8'd1);
        assert (MX_VECTOR_ACCESS_FAULT == 8'd2);
        assert (MX_VECTOR_ADDRESS_ERROR == 8'd3);
        assert (MX_VECTOR_ILLEGAL == 8'd4);
        assert (MX_VECTOR_ZERO_DIVIDE == 8'd5);
        assert (mx_m00_function_code(1'b0, 1'b0) == 3'b001);
        assert (mx_m00_function_code(1'b0, 1'b1) == 3'b010);
        assert (mx_m00_function_code(1'b1, 1'b0) == 3'b101);
        assert (mx_m00_function_code(1'b1, 1'b1) == 3'b110);
        assert (mx_m00_special_status_word(1'b1, 1'b0, 1'b1) ==
                16'h000d);
        assert (mx_m00_special_status_word(1'b0, 1'b1, 1'b0) ==
                16'h0012);
        assert (MX_VECTOR_CHK == 8'd6);
        assert (MX_VECTOR_TRAPCC == 8'd7);
        assert (MX_VECTOR_PRIVILEGE == 8'd8);
        assert (MX_VECTOR_TRACE == 8'd9);
        assert (MX_VECTOR_LINE_A == 8'd10);
        assert (MX_VECTOR_LINE_F == 8'd11);
        assert (MX_VECTOR_COPROTOCOL == 8'd13);
        assert (MX_VECTOR_FORMAT_ERROR == 8'd14);
        assert (MX_VECTOR_UNINITIALIZED_IRQ == 8'd15);
        assert (MX_VECTOR_SPURIOUS == 8'd24);
        assert (MX_VECTOR_TRAP_BASE == 8'd32);
        assert (MX_VECTOR_FP_BASE == 8'd48);
        assert (MX_VECTOR_MMU_CONFIG == 8'd56);
        assert (mx_sr_sanitize(16'hffff, MX_PROFILE_M00) == 16'ha71f);
        assert (mx_sr_sanitize(16'hffff, MX_PROFILE_M10) == 16'ha71f);
        assert (mx_sr_sanitize(16'hffff, MX_PROFILE_M20) == 16'hf71f);
        assert (mx_sr_sanitize(16'hffff, MX_PROFILE_M40) == 16'hf71f);
        assert (MX_VECTOR_AUTOVECTOR_BASE + 8'd7 == 8'd31);

        assert (!mx_interrupt_accepted(3'd0, 3'd0, 1'b0));
        assert (!mx_interrupt_accepted(3'd4, 3'd4, 1'b0));
        assert (mx_interrupt_accepted(3'd5, 3'd4, 1'b0));
        assert (mx_interrupt_accepted(3'd7, 3'd7, 1'b1));
        ...
        for (lhs = 0; lhs < 256; lhs = lhs + 1) begin
            for (rhs = 0; rhs < 256; rhs = rhs + 1) begin
                for (carry = 0; carry < 2; carry = carry + 1) begin
                    alu_value = mx_add(lhs, rhs, MX_OP_BYTE, carry[0], 1'b1);
                    ...
                    alu_value = mx_subtract(lhs, rhs, MX_OP_BYTE, carry[0], 1'b1);
        ...
        alu_value = mx_add(32'd0, 32'd0, MX_OP_LONG, 1'b0, 1'b0);
        assert (!alu_value.flags.z);
        alu_value = mx_subtract(32'd0, 32'd0, MX_OP_LONG, 1'b0, 1'b0);
        assert (!alu_value.flags.z);
        ...
        for (lhs_decimal = 0; lhs_decimal < 100;
            ...
            for (rhs_decimal = 0; rhs_decimal < 100;
                ...
                for (carry = 0; carry < 2; carry = carry + 1) begin
                    ...
                    expected_decimal = lhs_decimal - rhs_decimal - carry;
                    alu_value = mx_subtract_decimal_byte(
                        packed_lhs[7:0], packed_rhs[7:0], carry[0], 1'b1);
        ...
        alu_value = mx_add_decimal_byte(8'h00, 8'h00, 1'b0, 1'b0);
        assert (!alu_value.flags.z);
        alu_value = mx_subtract_decimal_byte(8'h00, 8'h00, 1'b0, 1'b0);
        assert (!alu_value.flags.z);
                ...
                case (condition[3:0])
                    ...
                    default: expected_condition = condition_flags.z ||
                                           (condition_flags.n !=
                                            condition_flags.v);
                endcase
                assert (mx_condition_true(condition[3:0], condition_flags) ==
                        expected_condition);
            end
        end

        $display("PASS: MX68K SR/vectors, conditions, binary and packed-BCD ALU");
        $finish;
    end
endmodule
