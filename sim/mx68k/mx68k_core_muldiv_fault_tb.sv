module mx68k_core_muldiv_fault_tb;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            cycles <= cycles + 1;
            if (cycles > 4000)
                $fatal(1, "MUL/DIV fault matrix timed out");
        end
    end
    ...
    task automatic run_extension_fault(input integer operation,
        ...
        begin
            ...
            opcode = operation_base(operation) | 16'h0200 |
                     (second_word ? 16'h0039 : 16'h0038);
            rst_n = 1'b0;
            ...
            assert (debug_data_registers[1*32 +: 32] == 32'd0);
    ...
    task automatic run_data_fault(input integer operation);
        ...
        begin
            ...
            opcode = operation_base(operation) | 16'h0018; // (A0)+,D0
            rst_n = 1'b0;
            repeat (2) @(negedge clk);
            ...
            set_instruction_word(16'h106, 16'h203c); // MOVE.L #value,D0
            ...
            assert (debug_data_registers[0*32 +: 32] == destination);
    ...
    initial begin
        ...
        for (int operation = 0; operation < 4; operation++) begin
            ...
            run_data_fault(operation);
        end

        $display("PASS: MULU/MULS/DIVU/DIVS extension and operand-read faults");
        $finish;
    end
endmodule
