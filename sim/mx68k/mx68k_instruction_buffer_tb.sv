module mx68k_instruction_buffer_tb;
    import mx68k_pkg::*;
    ...
    mx68k_instruction_buffer #(.DEPTH_WORDS(DEPTH_WORDS)) buffer (
        .clk, .rst_n, .flush,
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            cycles <= cycles + 1;
            if (cycles > 2000)
                $fatal(1, "MX68K instruction buffer test timed out");
        end
    end
    ...
    initial begin
        ...
        sample_uop.uop_class = MX_UCLASS_LOAD;
        sample_uop.opcode = MX_UOP_LOAD;
        sample_uop.size = MX_OP_LONG;
        sample_uop.profile = MX_PROFILE_M00;
        sample_uop.flags_write = MX_FLAG_ALL;
        assert (mx_uop_is_memory(sample_uop));
        assert (!mx_uop_is_control(sample_uop));
        assert (mx_uop_writes_flags(sample_uop));
        assert (MX_FLAG_ALL == 5'b11111);
        assert ((MX_FLAG_X | MX_FLAG_N | MX_FLAG_Z | MX_FLAG_V | MX_FLAG_C) ==
                MX_FLAG_ALL);

        repeat (4) @(negedge clk);
        rst_n = 1'b1;

        send_word(32'h0000_0100, 16'h4e71, MX_FAULT_NONE);
        send_word(32'h0000_0102, 16'h6000, MX_FAULT_NONE);
        send_word(32'h0000_0104, 16'h0004, MX_FAULT_NONE);
        send_word(32'h0000_0106, 16'h4afc, MX_FAULT_NONE);
        ...
        send_word(32'h0000_0108, 16'h4e71, MX_FAULT_NONE);
        assert (window_pc == 32'h0000_0108);

        // Flush creates a new PC stream and faults remain attached to their
        // exact extension word.
        flush_buffer();
        send_word(32'h0000_0200, 16'h6000, MX_FAULT_NONE);
        send_word(32'h0000_0202, 16'h0000, MX_FAULT_ACCESS);
        assert (window_count == 2);
        assert (window_faults[0*4 +: 4] == MX_FAULT_NONE);
        assert (window_faults[1*4 +: 4] == MX_FAULT_ACCESS);
        ...
        for (word_index = 0; word_index < DEPTH_WORDS; word_index = word_index + 1)
            send_word(32'h0000_0300 + word_index*2,
                      16'(16'h1000 + word_index), MX_FAULT_NONE);
        ...
        $display("PASS: MX68K typed uops and variable instruction buffer");
        $finish;
    end
endmodule
