module mx68k_decode_frontend_tb;
    import mx68k_pkg::*;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            cycles <= cycles + 1;
            if (cycles > 5000)
                $fatal(1, "MX68K integrated decode frontend test timed out");
        end
    end
    ...
    initial begin
        ...
        assert (instruction_uop.opcode == MX_UOP_BRANCH);
        ...
        wait_for_instruction();
        assert (instruction_uop.opcode == MX_UOP_STOP);
        ...
        assert (instruction_exception.exception_class == MX_EXC_FETCH);
        assert (instruction_exception.vector == MX_VECTOR_ACCESS_FAULT);
        ...
        wait_for_instruction();
        assert (!instruction_uop.valid && instruction_exception.valid);
        assert (instruction_exception.vector == MX_VECTOR_ADDRESS_ERROR);
        ...
        $display("PASS: MX68K fetch-to-decode integration, stalls, redirects and faults");
        $finish;
    end
endmodule
