module mx68k_fetch_frontend_tb;
    import mx68k_pkg::*;
    ...
    mx68k_fetch_frontend #(.SOURCE_ID(4'h2)) frontend (
        .clk, .rst_n,
    ...
    task automatic expect_word(
        ...
        input mx_mem_fault_t expected_fault,
        input integer blocked_cycles
    );
        integer blocked;
        logic [31:0] held_pc;
        logic [15:0] held_data;
        mx_mem_fault_t held_fault;
        begin
            while (!word_valid)
                @(negedge clk);
            held_pc = word_pc;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            cycles <= cycles + 1;
            if (cycles > 5000)
                $fatal(1, "MX68K fetch frontend test timed out");
        end
    end
    ...
    initial begin
        ...
        expect_word(32'h0000_000e, 16'ha0a1, MX_FAULT_NONE, 3);
        expect_word(32'h0000_0010, 16'hb0b1, MX_FAULT_NONE, 1);
        ...
        expect_word(32'h0000_0080, 16'hdead, MX_FAULT_NONE, 2);
