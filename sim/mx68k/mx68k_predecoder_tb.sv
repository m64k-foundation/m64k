module mx68k_predecoder_tb;
    ...
    mx_uop_t decode_uop;
    mx_exception_t decode_exception;
    logic [15:0] test_opcode;

    mx68k_predecoder #(.WINDOW_WORDS(WINDOW_WORDS)) decoder (.*);

    task automatic set_window(
        input mx_profile_t new_profile,
        input logic [31:0] pc,
    ...
    initial begin
        ...
        set_window(MX_PROFILE_M00, 32'h100, COUNT_WIDTH'(1),
                   16'h4e71, 16'd0, 16'd0);
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_NOP);
        assert (decode_uop.instruction_id == MX_INSN_NOP);
        assert (decode_uop.sequential_pc == 32'h102);

        set_window(MX_PROFILE_M00, 32'h180, COUNT_WIDTH'(1),
                   16'h76a5, 16'd0, 16'd0);
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_MOVE);
        assert (decode_uop.instruction_id == MX_INSN_MOVEQ);
        assert (decode_uop.size == MX_OP_LONG);
        assert (decode_uop.destination.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.destination.index == 3);
        assert (decode_uop.immediate == 32'hffff_ffa5);
        assert (decode_uop.flags_write == (MX_FLAG_N | MX_FLAG_Z |
                                           MX_FLAG_V | MX_FLAG_C));

        set_window(MX_PROFILE_M00, 32'h190, COUNT_WIDTH'(1),
                   16'h4e63, 16'd0, 16'd0); // MOVE A3,USP
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_MOVE_USP);
        assert (decode_uop.opcode == MX_UOP_MOVE);
        assert (decode_uop.size == MX_OP_LONG && decode_uop.privileged);
        assert (decode_uop.source_a.kind == MX_OPERAND_ADDRESS_REGISTER);
        assert (decode_uop.source_a.index == 3);
        assert (decode_uop.destination.kind == MX_OPERAND_USP);
        assert (decode_uop.flags_write == 0);

        set_window(MX_PROFILE_M00, 32'h192, COUNT_WIDTH'(1),
                   16'h4e6f, 16'd0, 16'd0); // MOVE USP,A7
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_MOVE_USP);
        assert (decode_uop.source_a.kind == MX_OPERAND_USP);
        assert (decode_uop.destination.kind == MX_OPERAND_ADDRESS_REGISTER);
        assert (decode_uop.destination.index == 7);
        assert (decode_uop.flags_write == 0);

        set_window(MX_PROFILE_M00, 32'h1a0, COUNT_WIDTH'(3),
        ...
        assert (decode_uop.opcode == MX_UOP_MOVE);
        assert (decode_uop.size == MX_OP_BYTE && decode_uop.memory_write);
        assert (decode_uop.immediate == 32'h4f);
        assert (decode_uop.memory_address == 32'hffff_f800);
        assert (decode_uop.source_ea.kind == MX_EA_IMMEDIATE);
        assert (decode_uop.destination_ea.kind == MX_EA_ABSOLUTE_LONG);
        assert (decode_uop.sequential_pc == 32'h1a8);
        window_faults[15:12] = MX_FAULT_BUS;
        #1;
        assert (!decode_uop.valid && decode_exception.valid);
        assert (decode_words == 4 && decode_exception.next_pc == 32'h1a6);

        // First instructions emitted by GCC for the real Mackerel-F startup.
        set_window(MX_PROFILE_M00, 32'hff00f0, COUNT_WIDTH'(1),
                   16'h2f0a, 16'd0, 16'd0); // MOVE.L A2,-(SP)
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_MOVE && decode_uop.memory_write);
        assert (decode_uop.source_ea.kind == MX_EA_ADDRESS_REGISTER);
        assert (decode_uop.source_ea.register_index == 2);
        assert (decode_uop.destination_ea.kind == MX_EA_PREDECREMENT);
        assert (decode_uop.destination_ea.register_index == 7);

        set_window(MX_PROFILE_M00, 32'hff00f4, COUNT_WIDTH'(3),
                   16'h4eb9, 16'h00ff, 16'h03ac); // JSR abs.l
        assert (decode_valid && decode_words == 3 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_JUMP_SUBROUTINE);
        assert (decode_uop.destination_ea.kind == MX_EA_ABSOLUTE_LONG);

        set_window(MX_PROFILE_M00, 32'hff20d4, COUNT_WIDTH'(2),
                   16'h802a, 16'h000c, 16'd0); // OR.B 12(A2),D0
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_OR);
        assert (decode_uop.size == MX_OP_BYTE);
        assert (decode_uop.source_ea.kind == MX_EA_DISPLACEMENT);

        set_window(MX_PROFILE_M00, 32'hff0b06, COUNT_WIDTH'(1),
                   16'hd1c0, 16'd0, 16'd0); // ADDA.L D0,A0
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_ADD);
        assert (decode_uop.size == MX_OP_LONG);
        assert (decode_uop.destination.kind == MX_OPERAND_ADDRESS_REGISTER);
        assert (decode_uop.destination.index[2:0] == 3'd0);

        set_window(MX_PROFILE_M00, 32'hff61c4, COUNT_WIDTH'(2),
                   16'hc0ef, 16'h000a, 16'd0); // MULU.W 10(SP),D0
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_MULTIPLY);
        assert (!decode_uop.condition[0]);
        assert (decode_uop.source_ea.kind == MX_EA_DISPLACEMENT);

        set_window(MX_PROFILE_M00, 32'hff61c8, COUNT_WIDTH'(1),
                   16'h85c1, 16'd0, 16'd0); // DIVS.W D1,D2
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_DIVIDE);
        assert (decode_uop.condition[0]);
        assert (decode_uop.source_ea.kind == MX_EA_DATA_REGISTER);
        assert (decode_uop.destination.index[2:0] == 3'd2);

        set_window(MX_PROFILE_M00, 32'hff61ca, COUNT_WIDTH'(1),
                   16'h43bc, 16'd0, 16'd0); // CHK.W #10,D1
        assert (!decode_valid && decode_need_more);
        set_window(MX_PROFILE_M00, 32'hff61ca, COUNT_WIDTH'(2),
                   16'h43bc, 16'h000a, 16'd0);
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_CHK_W);
        assert (decode_uop.opcode == MX_UOP_CHECK_BOUNDS);
        assert (decode_uop.size == MX_OP_WORD);
        assert (decode_uop.source_ea.kind == MX_EA_IMMEDIATE);
        assert (decode_uop.destination.index[2:0] == 3'd1);
        assert (decode_uop.exception_vector == MX_VECTOR_CHK);

        set_window(MX_PROFILE_M00, 32'hff61ce, COUNT_WIDTH'(1),
                   16'h4188, 16'd0, 16'd0); // CHK.W A0,D0 is illegal
        assert (decode_valid && !decode_uop.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);

        set_window(MX_PROFILE_M00, 32'hff4b06, COUNT_WIDTH'(2),
                   16'hd5ae, 16'hff94, 16'd0); // ADD.L D2,-108(A6)
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_ADD);
        assert (decode_uop.size == MX_OP_LONG && decode_uop.memory_write);
        assert (decode_uop.source_a.index[2:0] == 3'd2);
        assert (decode_uop.destination_ea.kind == MX_EA_DISPLACEMENT);

        set_window(MX_PROFILE_M00, 32'h00019798, COUNT_WIDTH'(1),
                   16'h9986, 16'd0, 16'd0); // SUBX.L D6,D4
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_SUBTRACT_EXTEND);
        assert (decode_uop.size == MX_OP_LONG && decode_uop.condition[0]);
        assert (decode_uop.source_a.index[2:0] == 3'd6);
        assert (decode_uop.destination.index[2:0] == 3'd4);
        assert (decode_uop.flags_read == (MX_FLAG_X | MX_FLAG_Z));
        assert (decode_uop.flags_write == MX_FLAG_ALL);

        set_window(MX_PROFILE_M00, 32'h0001979a, COUNT_WIDTH'(1),
                   16'hd108, 16'd0, 16'd0); // ADDX.B -(A0),-(A0)
        assert (decode_valid && decode_uop.valid &&
                !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_ADDX_MEM_B);
        assert (decode_uop.opcode == MX_UOP_ADD_EXTEND);
        assert (decode_uop.size == MX_OP_BYTE);
        assert (decode_uop.source_ea.kind == MX_EA_PREDECREMENT &&
        ...
        set_window(MX_PROFILE_M00, 32'h0001979c, COUNT_WIDTH'(1),
                   16'h974a, 16'd0, 16'd0); // SUBX.W -(A2),-(A3)
        assert (decode_valid && decode_uop.valid &&
                !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_SUBX_MEM_W);
        assert (decode_uop.opcode == MX_UOP_SUBTRACT_EXTEND &&
        ...
        set_window(MX_PROFILE_M00, 32'h0001979e, COUNT_WIDTH'(1),
                   16'hc501, 16'd0, 16'd0); // ABCD D1,D2
        assert (decode_valid && decode_uop.valid &&
                !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_ABCD);
        assert (decode_uop.opcode == MX_UOP_ADD_EXTEND);
        assert (decode_uop.size == MX_OP_BYTE && decode_uop.condition[1]);
        ...
        set_window(MX_PROFILE_M00, 32'h000197a0, COUNT_WIDTH'(1),
                   16'h870a, 16'd0, 16'd0); // SBCD -(A2),-(A3)
        assert (decode_valid && decode_uop.valid &&
                !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_SBCD);
        assert (decode_uop.opcode == MX_UOP_SUBTRACT_EXTEND);
        ...
        set_window(MX_PROFILE_M00, 32'h000197a2, COUNT_WIDTH'(1),
                   16'h4823, 16'd0, 16'd0); // NBCD -(A3)
        assert (decode_valid && decode_uop.valid &&
                !decode_exception.valid);
        assert (decode_uop.instruction_id == MX_INSN_NBCD);
        assert (decode_uop.opcode == MX_UOP_SUBTRACT_EXTEND);
        assert (decode_uop.size == MX_OP_BYTE);
        ...
        set_window(MX_PROFILE_M00, 32'h000197a4, COUNT_WIDTH'(1),
                   16'h4808, 16'd0, 16'd0); // NBCD A0 is illegal
        assert (decode_valid && !decode_uop.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);

        set_window(MX_PROFILE_M00, 32'h0008dda6, COUNT_WIDTH'(1),
                   16'h904c, 16'd0, 16'd0); // SUB.W A4,D0
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_SUBTRACT);
        assert (decode_uop.size == MX_OP_WORD);
        assert (decode_uop.source_a.kind == MX_OPERAND_ADDRESS_REGISTER);
        ...
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_ADD_EXTEND);
        assert (decode_uop.size == MX_OP_WORD && !decode_uop.condition[0]);
        assert (decode_uop.source_a.index[2:0] == 3'd1);
        assert (decode_uop.destination.index[2:0] == 3'd2);

        set_window(MX_PROFILE_M00, 32'hff3d1e, COUNT_WIDTH'(1),
                   16'hb400, 16'd0, 16'd0); // CMP.B D0,D2
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_COMPARE);
        assert (decode_uop.size == MX_OP_BYTE);
        assert (decode_uop.source_a.index[2:0] == 3'd0);
        assert (decode_uop.destination.index[2:0] == 3'd2);

        set_window(MX_PROFILE_M00, 32'hff3fe8, COUNT_WIDTH'(3),
                   16'h0280, 16'h0000, 16'h0003); // ANDI.L #3,D0
        assert (decode_valid && decode_words == 3 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_AND);
        assert (decode_uop.immediate == 3);
        assert (decode_uop.destination.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.sequential_pc == 32'hff3fee);

        set_window(MX_PROFILE_M00, 32'hff4034, COUNT_WIDTH'(2),
                   16'h51c9, 16'hfffc, 16'd0); // DBF D1,$ff4032
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_DBCC);
        assert (decode_uop.condition == 4'h1);
        assert (decode_uop.destination.index[2:0] == 3'd1);
        assert (decode_uop.immediate == 32'hff4032);
        assert (decode_uop.sequential_pc == 32'hff4038);

        set_window(MX_PROFILE_M00, 32'hff023c, COUNT_WIDTH'(3),
        ...
        set_window(MX_PROFILE_M00, 32'hff3f68, COUNT_WIDTH'(2),
                   16'h48e7, 16'h3020, 16'd0); // MOVEM.L D2-D3/A2,-(SP)
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_MOVE_MULTIPLE);
        assert (!decode_uop.condition[0] && decode_uop.memory_write);
        assert (decode_uop.size == MX_OP_LONG);
        assert (decode_uop.immediate[15:0] == 16'h3020);
        assert (decode_uop.source_ea.kind == MX_EA_PREDECREMENT);

        set_window(MX_PROFILE_M00, 32'hff3fa6, COUNT_WIDTH'(2),
                   16'h4cdf, 16'h040c, 16'd0); // MOVEM.L (SP)+,D2-D3/A2
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_MOVE_MULTIPLE);
        assert (decode_uop.condition[0] && !decode_uop.memory_write);
        assert (decode_uop.source_ea.kind == MX_EA_POSTINCREMENT);

        set_window(MX_PROFILE_M00, 32'hff3a14, COUNT_WIDTH'(2),
                   16'h4e56, 16'hfff8, 16'd0); // LINK A6,#-8
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_LINK);
        assert (decode_uop.source_ea.register_index == 3'd6);
        assert (decode_uop.immediate == 32'hfffffff8);

        set_window(MX_PROFILE_M00, 32'hff3a62, COUNT_WIDTH'(2),
        ...
        set_window(MX_PROFILE_M00, 32'hff06fe, COUNT_WIDTH'(4),
                   16'h0839, 16'h0005, 16'h00ff); // BTST #5,$00fff90a
        window_words[63:48] = 16'hf90a;
        #1;
        assert (decode_valid && decode_words == 4 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_BIT_TEST);
        assert (decode_uop.size == MX_OP_BYTE);
        ...
        set_window(MX_PROFILE_M00, 32'h00003dde, COUNT_WIDTH'(1),
                   16'h03d0, 16'd0, 16'd0); // BSET D1,(A0)
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_BIT_TEST);
        assert (decode_uop.size == MX_OP_BYTE);
        assert (decode_uop.source_a.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.source_a.index[2:0] == 3'd1);
        assert (decode_uop.destination_ea.kind == MX_EA_INDIRECT);
        assert (decode_uop.condition[2:0] == 3'b111);

        set_window(MX_PROFILE_M00, 32'hff5524, COUNT_WIDTH'(1),
                   16'h44c1, 16'd0, 16'd0); // MOVE.W D1,CCR
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_MOVE);
        assert (decode_uop.source_ea.kind == MX_EA_DATA_REGISTER);
        assert (decode_uop.destination.kind == MX_OPERAND_SR);
        assert (decode_uop.condition[0]);

        set_window(MX_PROFILE_M00, 32'h00000400, COUNT_WIDTH'(2),
        ...
        set_window(MX_PROFILE_M00, 32'h0001a6ba, COUNT_WIDTH'(1),
                   16'h40c7, 16'd0, 16'd0); // MOVE.W SR,D7
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_MOVE && !decode_uop.privileged);
        assert (decode_uop.source_a.kind == MX_OPERAND_SR);
        assert (decode_uop.destination.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.destination.index[2:0] == 3'd7);

        set_window(MX_PROFILE_M00, 32'h0008d1a4, COUNT_WIDTH'(2),
                   16'h50ea, 16'h0004, 16'd0); // ST 4(A2)
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_SET_CONDITION);
        assert (decode_uop.condition == 4'h0);
        assert (decode_uop.size == MX_OP_BYTE && decode_uop.memory_write);
        assert (decode_uop.destination_ea.kind == MX_EA_DISPLACEMENT);

        set_window(MX_PROFILE_M00, 32'h00000400, COUNT_WIDTH'(1),
                   16'h46c8, 16'd0, 16'd0); // reserved MOVE.W A0,SR
        assert (decode_valid && decode_words == 1 && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);

        set_window(MX_PROFILE_M00, 32'hff06a8, COUNT_WIDTH'(1),
                   16'h4882, 16'd0, 16'd0); // EXT.W D2
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_SIGN_EXTEND);
        assert (decode_uop.size == MX_OP_WORD);
        assert (decode_uop.destination.index[2:0] == 3'd2);

        set_window(MX_PROFILE_M00, 32'hff3a56, COUNT_WIDTH'(1),
                   16'h4e5e, 16'd0, 16'd0); // UNLK A6
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_UNLINK);
        assert (decode_uop.source_ea.register_index == 3'd6);

        set_window(MX_PROFILE_M00, 32'hff405a, COUNT_WIDTH'(1),
                   16'he188, 16'd0, 16'd0); // LSL.L #8,D0
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_SHIFT);
        assert (decode_uop.size == MX_OP_LONG);
        assert (decode_uop.condition == 4'b1010);
        assert (decode_uop.immediate == 8);

        set_window(MX_PROFILE_M00, 32'hff4060, COUNT_WIDTH'(1),
                   16'h4840, 16'd0, 16'd0); // SWAP D0
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_SWAP);
        assert (decode_uop.destination.index[2:0] == 3'd0);

        set_window(MX_PROFILE_M00, 32'hff4062, COUNT_WIDTH'(1),
                   16'hc141, 16'd0, 16'd0); // EXG D1,D0
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_EXCHANGE);
        assert (decode_uop.size == MX_OP_LONG && decode_uop.flags_write == 0);
        assert (decode_uop.source_a.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.source_a.index[2:0] == 3'd1);
        assert (decode_uop.destination.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.destination.index[2:0] == 3'd0);

        set_window(MX_PROFILE_M00, 32'hff4064, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'hff4066, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'hff4068, COUNT_WIDTH'(1),
                   16'h4ac3, 16'd0, 16'd0); // TAS D3
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_ATOMIC);
        assert (decode_uop.size == MX_OP_BYTE);
        assert (decode_uop.destination.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.destination.index[2:0] == 3'd3);
        assert (!decode_uop.memory_atomic && !decode_uop.memory_write);
        assert (decode_uop.flags_write ==
                (MX_FLAG_N | MX_FLAG_Z | MX_FLAG_V | MX_FLAG_C));

        set_window(MX_PROFILE_M00, 32'hff406a, COUNT_WIDTH'(1),
                   16'h4ae8, 16'd0, 16'd0); // TAS 4(A0), missing extension
        assert (!decode_valid && decode_need_more && decode_words == 0);

        set_window(MX_PROFILE_M00, 32'hff406a, COUNT_WIDTH'(2),
        ...
        window_faults[7:4] = MX_FAULT_BUS;
        #1;
        assert (decode_valid && !decode_uop.valid && decode_words == 2);
        assert (decode_exception.vector == MX_VECTOR_ACCESS_FAULT);
        assert (decode_exception.next_pc == 32'hff406c);

        set_window(MX_PROFILE_M00, 32'hff406e, COUNT_WIDTH'(1),
                   16'h4ac8, 16'd0, 16'd0); // TAS A0 -- illegal
        assert (decode_valid && decode_words == 1 && decode_exception.valid);
        assert (!decode_uop.valid && decode_exception.vector == MX_VECTOR_ILLEGAL);

        set_window(MX_PROFILE_M00, 32'hff4070, COUNT_WIDTH'(2),
                   16'h4afa, 16'h0004, 16'd0); // TAS 4(PC) -- illegal
        assert (decode_valid && decode_words == 1 && decode_exception.valid);
        assert (!decode_uop.valid && decode_exception.vector == MX_VECTOR_ILLEGAL);

        set_window(MX_PROFILE_M00, 32'hff4072, COUNT_WIDTH'(2),
        ...
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_NEGATE);
        assert (decode_uop.size == MX_OP_LONG);
        assert (decode_uop.flags_write == MX_FLAG_ALL);

        set_window(MX_PROFILE_M00, 32'hff61f8, COUNT_WIDTH'(1),
                   16'h4242, 16'd0, 16'd0); // CLR.W D2
        assert (decode_valid && decode_words == 1 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_CLEAR);
        assert (decode_uop.size == MX_OP_WORD);
        assert (decode_uop.destination.kind == MX_OPERAND_DATA_REGISTER);
        assert (decode_uop.destination.index == 2);
        assert (decode_uop.destination_ea.kind == MX_EA_DATA_REGISTER);

        set_window(MX_PROFILE_M00, 32'hff03ac, COUNT_WIDTH'(2),
                   16'h4a2f, 16'h0007, 16'd0); // TST.B 7(SP)
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_TEST);
        assert (decode_uop.source_ea.kind == MX_EA_DISPLACEMENT);
        assert (decode_uop.source_ea.register_index == 7);

        set_window(MX_PROFILE_M00, 32'hff03b8, COUNT_WIDTH'(2),
                   16'h007c, 16'h0700, 16'd0); // ORI.W #$700,SR
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_OR && decode_uop.privileged);
        assert (decode_uop.destination.kind == MX_OPERAND_SR);

        set_window(MX_PROFILE_M00, 32'hff0100, COUNT_WIDTH'(3),
                   16'h45f9, 16'h00ff, 16'h5c12); // LEA abs.l,A2
        assert (decode_valid && decode_words == 3 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_CALCULATE_EA);
        assert (decode_uop.source_ea.kind == MX_EA_ABSOLUTE_LONG);
        assert (decode_uop.destination.kind == MX_OPERAND_ADDRESS_REGISTER);
        assert (decode_uop.destination.index == 2);

        set_window(MX_PROFILE_M00, 32'hff0110, COUNT_WIDTH'(2),
                   16'h487a, 16'h0010, 16'd0); // PEA 16(PC)
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_PUSH_EFFECTIVE_ADDRESS);
        assert (decode_uop.source_ea.kind == MX_EA_PC_DISPLACEMENT);
        assert (decode_uop.memory_write);

        set_window(MX_PROFILE_M00, 32'hff03c4, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'hff019a, COUNT_WIDTH'(1),
                   16'h4600, 16'd0, 16'd0); // NOT.B D0
        assert (decode_valid && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_NOT);
        assert (decode_uop.size == MX_OP_BYTE);
        assert (decode_uop.destination.kind == MX_OPERAND_DATA_REGISTER);

        set_window(MX_PROFILE_M00, 32'hff019c, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'hff01d8, COUNT_WIDTH'(3),
        ...
        set_window(MX_PROFILE_M00, 32'hff021a, COUNT_WIDTH'(3),
                   16'h4ef9, 16'h00ff, 16'h1418); // JMP abs.l
        assert (decode_valid && decode_words == 3 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_JUMP);
        assert (decode_uop.source_ea.kind == MX_EA_ABSOLUTE_LONG);

        set_window(MX_PROFILE_M00, 32'hff0190, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'hff0191, COUNT_WIDTH'(1),
                   16'h4a48, 16'd0, 16'd0); // TST.W A0 (M20+ only)
        assert (decode_valid && !decode_uop.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);

        set_window(MX_PROFILE_M00, 32'hff0191, COUNT_WIDTH'(2),
        ...
        set_window(MX_PROFILE_M00, 32'hff0191, COUNT_WIDTH'(3),
        ...
        set_window(MX_PROFILE_M00, 32'hff0192, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'h200, COUNT_WIDTH'(1),
                   16'hffff, 16'd0, 16'd0);
        assert (decode_valid && !decode_uop.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        assert (decode_exception.opcode == 16'hffff);

        set_window(MX_PROFILE_M00, 32'h300, COUNT_WIDTH'(1),
                   16'h4e4f, 16'd0, 16'd0);
        assert (decode_uop.opcode == MX_UOP_TRAP);
        assert (decode_uop.exception_vector == 8'd47);

        set_window(MX_PROFILE_M00, 32'h400, COUNT_WIDTH'(1),
                   16'h6002, 16'd0, 16'd0);
        assert (decode_uop.opcode == MX_UOP_BRANCH);
        assert (decode_uop.immediate == 32'h404 && decode_words == 1);

        set_window(MX_PROFILE_M00, 32'h500, COUNT_WIDTH'(1),
                   16'h61fc, 16'd0, 16'd0);
        assert (decode_uop.opcode == MX_UOP_BRANCH_SUBROUTINE);
        assert (decode_uop.immediate == 32'h4fe);

        set_window(MX_PROFILE_M00, 32'h600, COUNT_WIDTH'(1),
                   16'h6600, 16'd0, 16'd0);
        assert (!decode_valid && decode_need_more && decode_words == 0);

        set_window(MX_PROFILE_M00, 32'h600, COUNT_WIDTH'(2),
        ...
        set_window(MX_PROFILE_M00, 32'h700, COUNT_WIDTH'(1),
                   16'h60ff, 16'd0, 16'd0);
        assert (decode_valid && decode_words == 1);
        assert (decode_uop.immediate == 32'h701);

        set_window(MX_PROFILE_M20, 32'h700, COUNT_WIDTH'(2),
                   16'h60ff, 16'h0000, 16'd0);
        assert (!decode_valid && decode_need_more);

        set_window(MX_PROFILE_M20, 32'h700, COUNT_WIDTH'(3),
        ...
        window_faults[11:8] = MX_FAULT_PAGE;
        #1;
        ...
        set_window(MX_PROFILE_M00, 32'h800, COUNT_WIDTH'(1),
                   16'h4e72, 16'd0, 16'd0);
        assert (!decode_valid && decode_need_more);
        set_window(MX_PROFILE_M00, 32'h800, COUNT_WIDTH'(2),
                   16'h4e72, 16'h2700, 16'd0);
        assert (decode_valid && decode_words == 2);
        assert (decode_uop.opcode == MX_UOP_STOP && decode_uop.privileged);
        assert (decode_uop.immediate == 32'h2700);

        set_window(MX_PROFILE_M00, 32'h900, COUNT_WIDTH'(1),
                   16'h4e73, 16'd0, 16'd0);
        assert (decode_uop.opcode == MX_UOP_EXCEPTION_RETURN);
        assert (decode_uop.privileged && decode_uop.may_fault);

        set_window(MX_PROFILE_M00, 32'ha00, COUNT_WIDTH'(1),
                   16'h4e76, 16'd0, 16'd0);
        assert (decode_uop.opcode == MX_UOP_TRAP);
        assert (decode_uop.condition == 4'h9);
        assert (decode_uop.flags_read == MX_FLAG_V);

        set_window(MX_PROFILE_M00, 32'hb00, COUNT_WIDTH'(1),
                   16'h4e71, 16'd0, 16'd0);
        window_faults[3:0] = MX_FAULT_ALIGNMENT;
        #1;
        assert (decode_valid && decode_words == 1 && !decode_uop.valid);
        assert (decode_exception.vector == MX_VECTOR_ADDRESS_ERROR);
        assert (!decode_exception.rerunnable);

        // PRM 4-107..4-110: JMP, JSR and LEA accept control EAs only.
        // Dn and postincrement are deliberately holes in the broad masks.
        set_window(MX_PROFILE_M00, 32'hb10, COUNT_WIDTH'(1),
                   16'h4e80, 16'd0, 16'd0); // JSR D0 -- illegal
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb12, COUNT_WIDTH'(1),
                   16'h4ed8, 16'd0, 16'd0); // JMP (A0)+ -- illegal
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb14, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'hb20, COUNT_WIDTH'(2),
                   16'hc07c, 16'h0ff0, 16'd0); // AND.W #$0ff0,D0
        assert (decode_valid && decode_words == 2 && !decode_exception.valid);
        assert (decode_uop.opcode == MX_UOP_AND);
        assert (decode_uop.source_a.kind == MX_OPERAND_IMMEDIATE);
        assert (decode_uop.immediate == 32'h0000_0ff0);
        set_window(MX_PROFILE_M00, 32'hb24, COUNT_WIDTH'(1),
                   16'hc048, 16'd0, 16'd0); // AND.W A0,D0 -- illegal
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb26, COUNT_WIDTH'(1),
                   16'h8289, 16'd0, 16'd0); // OR.L A1,D1 -- illegal
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb28, COUNT_WIDTH'(1),
        ...
        set_window(MX_PROFILE_M00, 32'hb2a, COUNT_WIDTH'(1),
                   16'h1008, 16'd0, 16'd0); // MOVE.B A0,D0 -- illegal
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb2c, COUNT_WIDTH'(1),
                   16'h1040, 16'd0, 16'd0); // MOVE.B D0,A0 -- illegal
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb2e, COUNT_WIDTH'(3),
                   16'h48ba, 16'h0001, 16'h0004); // MOVEM.W D0,disp(PC)
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb34, COUNT_WIDTH'(2),
                   16'h4ca0, 16'h0001, 16'd0); // MOVEM.W -(A0),D0
        assert (decode_valid && !decode_uop.valid && decode_exception.valid);
        assert (decode_exception.vector == MX_VECTOR_ILLEGAL);
        set_window(MX_PROFILE_M00, 32'hb38, COUNT_WIDTH'(2),
            ...
            for (int ax_index = 0; ax_index < 8; ax_index++) begin
                for (int ay_index = 0; ay_index < 8; ay_index++) begin
                    test_opcode = {4'b1011, ax_index[2:0], 1'b1,
                                   ...
                                   ay_index[2:0]};
                    set_window(MX_PROFILE_M00, 32'hc00,
                               COUNT_WIDTH'(1), test_opcode, 16'd0, 16'd0);
                    assert (decode_valid && decode_words == 1 &&
                            !decode_exception.valid && decode_uop.valid);
                    assert (decode_uop.opcode == MX_UOP_COMPARE_MEMORY);
                    assert (decode_uop.size ==
                            mx_operand_size_t'(size_index));
        ...
        for (int ax_index = 0; ax_index < 8; ax_index++) begin
            for (int ay_index = 0; ay_index < 8; ay_index++) begin
                test_opcode = {4'b1011, ax_index[2:0], 3'b111, 3'b001,
                               ay_index[2:0]};
                set_window(MX_PROFILE_M00, 32'hc00, COUNT_WIDTH'(1),
                        ...
                        for (int count_field = 0; count_field < 8;
                            ...
                            for (int destination_index = 0;
                                ...
                                test_opcode = 16'(16'he000 |
                                    ...
                                    (kind_index << 3) |
                                    destination_index);
                                set_window(MX_PROFILE_M00, 32'hc40,
                                ...
                                assert (decode_uop.flags_read == MX_FLAG_X &&
        ...
        for (int size_index = 0; size_index < 3; size_index++) begin
            for (int ea_index = 0; ea_index < 64; ea_index++) begin
                ...
                set_window(MX_PROFILE_M00, 32'hc80,
                ...
                if (legal_tst_ea) begin
                    ...
                    assert (decode_uop.instruction_id == MX_INSN_TST &&
                            decode_uop.opcode == MX_UOP_TEST);
                    assert (decode_uop.size == mx_operand_size_t'(size_index));
                    assert (decode_uop.flags_write ==
                            (MX_FLAG_N | MX_FLAG_Z | MX_FLAG_V | MX_FLAG_C));
                    assert (!decode_uop.memory_write);
            ...
            for (int size_index = 0; size_index < 3; size_index++) begin
                for (int ea_index = 0; ea_index < 64; ea_index++) begin
                    ...
                    test_opcode = 16'(opcode_base | (size_index << 6) |
                                      ea_index);
                    set_window(MX_PROFILE_M00, 32'hcc0,
                               COUNT_WIDTH'(1 + extension_count), test_opcode,
                               16'h0000, 16'h0220);
                ...
                for (int size_index = 0; size_index < 3; size_index++) begin
                    for (int ea_index = 0; ea_index < 64; ea_index++) begin
                        ...
                        test_opcode = 16'(16'h5000 | (quick_field << 9) |
                            ...
                            ea_index);
                        set_window(MX_PROFILE_M00, 32'hca0,
                            COUNT_WIDTH'(1 + extension_count), test_opcode,
                            16'h0000, 16'h0220);
                        if (legal_data_ea || legal_address_ea) begin
                            ...
                            assert (decode_words == 1 + extension_count);
                            assert (decode_uop.instruction_id ==
                                (quick_operation ? MX_INSN_SUBQ :
                                                   MX_INSN_ADDQ));
                            assert (decode_uop.opcode ==
                                (quick_operation ? MX_UOP_SUBTRACT :
                                                   MX_UOP_ADD));
                            assert (decode_uop.size ==
                                    mx_operand_size_t'(size_index));
                            assert (decode_uop.immediate ==
                            ...
                            assert (decode_uop.flags_write ==
                                    (legal_address_ea ? 5'd0 : MX_FLAG_ALL));
                            assert (decode_uop.memory_write ==
                                    (legal_data_ea && (ea_mode != 0)));
                            assert (decode_uop.may_fault ==
                                    (legal_data_ea && (ea_mode != 0)));
        ...
        for (int condition_index = 0; condition_index < 16;
            ...
            for (int ea_index = 0; ea_index < 64; ea_index++) begin
                ...
                test_opcode = 16'(16'h50c0 | (condition_index << 8) |
                                  ea_index);
                set_window(MX_PROFILE_M00, 32'hce0,
                           COUNT_WIDTH'(1 + extension_count), test_opcode,
                           16'h0000, 16'h0220);
        ...
        for (int operation_index = 0; operation_index < 8;
            ...
            for (int ea_index = 0; ea_index < 64; ea_index++) begin
                ...
                test_opcode = 16'('he0c0 | (operation_index << 8) |
                                  ea_index);
                set_window(MX_PROFILE_M00, 32'hd00,
                           COUNT_WIDTH'(1 + extension_count), test_opcode,
                           16'h0200, 16'h0000);
            ...
            for (int low_byte = 0; low_byte < 256; low_byte++) begin
                ...
                special_legal = ((operation_index == 0) ||
                                ...
                                ((low_byte == 8'h3c) ||
                                 (low_byte == 8'h7c));
                case (operation_index)
                    0: expected_id = MX_INSN_ORI;
                    1: expected_id = MX_INSN_ANDI;
                    2: expected_id = MX_INSN_SUBI;
                    3: expected_id = MX_INSN_ADDI;
                    4: expected_id = MX_INSN_EORI;
                    default: expected_id = MX_INSN_CMPI;
                endcase

                test_opcode = operation_base | 16'(low_byte);
                set_window(MX_PROFILE_M00, 32'h0000_0e00,
                ...
                if (generic_legal) begin
                    ...
                    case (operation_index)
                        0: assert (decode_uop.instruction_id ==
                            ((low_byte == 8'h3c) ? MX_INSN_ORI_CCR :
                                                   MX_INSN_ORI_SR));
                        1: assert (decode_uop.instruction_id ==
                            ((low_byte == 8'h3c) ? MX_INSN_ANDI_CCR :
                                                   MX_INSN_ANDI_SR));
                        default: assert (decode_uop.instruction_id ==
                            ((low_byte == 8'h3c) ? MX_INSN_EORI_CCR :
                                                   MX_INSN_EORI_SR));
                    endcase
                end else begin
            ...
            for (int size_index = 0; size_index < 3; size_index++) begin
                for (int destination_register = 0;
                    ...
                    for (int ea_index = 0; ea_index < 64; ea_index++) begin
                        ...
                        case (operation_index)
                            0: expected_id =
                                8'(MX_INSN_ADD_B + size_index);
                            1: expected_id = (size_index == 0) ?
                                MX_INSN_SUB_B : (size_index == 1) ?
                                MX_INSN_SUB_W : MX_INSN_SUB_L;
                            2: expected_id = (size_index == 0) ?
                                MX_INSN_AND_B : (size_index == 1) ?
                                MX_INSN_AND_W : MX_INSN_AND_L;
                            3: expected_id =
                                8'(MX_INSN_OR_B + size_index);
                            default: expected_id = (size_index == 0) ?
                                MX_INSN_CMP_B : (size_index == 1) ?
                                MX_INSN_CMP_W : MX_INSN_CMP_L;
                        endcase
                        test_opcode = {operation_nibble,
                            destination_register[2:0], 1'b0,
                            size_index[1:0], ea_index[5:0]};
                        set_window(MX_PROFILE_M00, 32'h0000_1000,
                    ...
                    for (int ea_index = 0; ea_index < 64; ea_index++) begin
                        ...
                        legal_source = (ea_mode <= 6) ||
                                       ((ea_mode == 7) &&
                                        (ea_register <= 4));
                        case (operation_index)
                            0: expected_id = size_index ? MX_INSN_ADDA_L :
                                                        MX_INSN_ADDA_W;
                            1: expected_id = size_index ? MX_INSN_SUBA_L :
                                                        MX_INSN_SUBA_W;
                            default: expected_id = size_index ? MX_INSN_CMPA_L :
                                                              MX_INSN_CMPA_W;
                        endcase
                        test_opcode = {operation_nibble,
                            destination_register[2:0], size_index[0], 2'b11,
                            ea_index[5:0]};
                        set_window(MX_PROFILE_M00, 32'h0000_1200,
                                   COUNT_WIDTH'(1 + extension_words),
                                   test_opcode, 16'h0000, 16'h0000);
                        if (legal_source) begin
                            legal_count = legal_count + 1;
                            ...
                            assert (decode_uop.destination.index[2:0] ==
                                    destination_register[2:0]);
                            assert (decode_uop.size ==
                                    (size_index ? MX_OP_LONG : MX_OP_WORD));
                            assert (!decode_uop.memory_write);
                            assert (decode_uop.flags_write ==
                                    ((operation_index == 2) ?
                                     (MX_FLAG_N | MX_FLAG_Z |
                                      MX_FLAG_V | MX_FLAG_C) : 5'd0));
                        end else begin
                            illegal_count = illegal_count + 1;
                            assert (decode_valid && !decode_uop.valid &&
                                    decode_exception.valid);
                        ...
                        if (((operation_index == 0) ||
                            ...
                            if (operation_index == 0)
                                expected_overlap_id = (ea_mode == 0) ?
                                    8'(MX_INSN_ADDX_REG_B + size_index) :
                                    8'(MX_INSN_ADDX_MEM_B + size_index);
                            else
                                expected_overlap_id = (ea_mode == 0) ?
                                    8'(MX_INSN_SUBX_REG_B + size_index) :
                                    8'(MX_INSN_SUBX_MEM_B + size_index);
                        end else if ((operation_index == 2) &&
                                     (size_index == 0) &&
                                     ((ea_mode == 0) || (ea_mode == 1))) begin
                            overlap_legal = 1'b1;
                            expected_overlap_id = MX_INSN_ABCD;
                        end else if ((operation_index == 2) &&
                                     (size_index == 1) &&
                                     (ea_mode == 0)) begin
                            overlap_legal = 1'b1;
                            expected_overlap_id = MX_INSN_EXG_DD;
                        end else if ((operation_index == 2) &&
                                     (size_index == 1) &&
                                     (ea_mode == 1)) begin
                            overlap_legal = 1'b1;
                            expected_overlap_id = MX_INSN_EXG_AA;
                        end else if ((operation_index == 2) &&
                                     (size_index == 2) &&
                                     (ea_mode == 1)) begin
                            overlap_legal = 1'b1;
                            expected_overlap_id = MX_INSN_EXG_DA;
                        end else if ((operation_index == 3) &&
                                     (size_index == 0) &&
                                     ((ea_mode == 0) || (ea_mode == 1))) begin
                            overlap_legal = 1'b1;
                            expected_overlap_id = MX_INSN_SBCD;
                        end else if ((operation_index == 4) &&
                                     (ea_mode == 1)) begin
                            overlap_legal = 1'b1;
                            expected_overlap_id =
                                8'(MX_INSN_CMPM_B + size_index);
                        end
        ...
        for (int operation_index = 0; operation_index < 4;
            ...
            case (operation_index)
                0: begin
                    ...
                    expected_id = MX_INSN_MULU;
                end
                1: begin
                    operation_base = 16'hc1c0;
                    expected_id = MX_INSN_MULS;
                end
                2: begin
                    operation_base = 16'h80c0;
                    expected_id = MX_INSN_DIVU;
                end
                default: begin
                    operation_base = 16'h81c0;
                    expected_id = MX_INSN_DIVS;
                end
            endcase
            for (int destination_register = 0;
                 destination_register < 8; destination_register++) begin
                for (int ea_index = 0; ea_index < 64; ea_index++) begin
                    ...
                    set_window(MX_PROFILE_M00, 32'h0000_1600,
                               COUNT_WIDTH'(1 + extension_words),
                    ...
                    if (legal_source) begin
                        ...
                        assert (decode_uop.instruction_id == expected_id);
                        assert (decode_uop.opcode ==
                            ((operation_index < 2) ? MX_UOP_MULTIPLY :
                                                     MX_UOP_DIVIDE));
                        assert (decode_uop.size == MX_OP_WORD);
                        assert (decode_uop.condition[0] ==
                                operation_index[0]);
        ...
        $display("PASS: MX68K generated M00 predecoder and profile branch lengths");
        $finish;
    end
endmodule
