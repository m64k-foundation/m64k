module m64k_core_unary_matrix_tb;
    import m64k_pkg::*;
    ...
    function automatic logic [7:0] sample_bcd_operand(input integer variant);
        case (variant & 3)
            0: return 8'h00;
            1: return 8'h01;
            2: return 8'h50;
            default: return 8'h99;
        endcase
    endfunction
    ...
    task automatic run_unary_case(input integer unary_index,
        ...
        integer decimal_operand;
        integer decimal_magnitude;
        integer decimal_result;
        begin
            ...
            mask = size_mask(size_index);
            sign_bit = sign_mask(size_index);
            initial_ccr = 5'b1_1111;
            operand = sample_operand(size_index, ea_register);
            if (unary_index == 3) begin
                operand = {24'ha5a500, sample_bcd_operand(variant)};
                initial_ccr[M64K_SR_X] = variant[0];
                initial_ccr[M64K_SR_N] = variant[1];
                initial_ccr[M64K_SR_Z] = variant[1];
                initial_ccr[M64K_SR_V] = !variant[0];
                initial_ccr[M64K_SR_C] = !variant[1];
            end
            ...
            case (unary_index)
                0: begin
                    ...
                    expected_instruction_id = M64K_INSN_CLR;
                    result_sized = 32'd0;
                    expected_ccr = 5'b1_0100;
                end
                1: begin
                    opcode_base = 16'h4400;
                    expected_instruction_id = M64K_INSN_NEG;
                ...
                2: begin
                    ...
                    expected_instruction_id = M64K_INSN_NOT;
                ...
                default: begin
                    ...
                    // (1992), NBCD, pp. 4-140--4-142: packed-decimal
                    // 0-destination-X, X=C=borrow, cumulative Z; N/V are
                    // undefined.  M64K deterministically preserves N/V.
                    opcode_base = 16'h4800;
                    expected_instruction_id = M64K_INSN_NBCD;
                    decimal_operand = 10 * operand_sized[7:4] +
                                      operand_sized[3:0];
                    decimal_magnitude = decimal_operand +
                                        initial_ccr[M64K_SR_X];
                    if (decimal_magnitude == 0) begin
                        decimal_result = 0;
                        expected_ccr = initial_ccr;
                        expected_ccr[M64K_SR_X] = 1'b0;
                        expected_ccr[M64K_SR_C] = 1'b0;
                    end else begin
                        decimal_result = 100 - decimal_magnitude;
                        expected_ccr = initial_ccr;
                        expected_ccr[M64K_SR_X] = 1'b1;
                        expected_ccr[M64K_SR_C] = 1'b1;
                    end
                    result_sized = 32'(((decimal_result / 10) << 4) |
                                       (decimal_result % 10));
                    expected_ccr[M64K_SR_Z] =
                        initial_ccr[M64K_SR_Z] && (result_sized == 0);
                end
            endcase
            ...
            set_instruction_word(16'h106, 16'h44fc); // MOVE.W #flags,CCR
            set_instruction_word(16'h108, {11'd0, initial_ccr});

            unary_opcode = 16'(opcode_base |
                               ((unary_index == 3) ? 0 : (size_index << 6)) |
                ...
                for (int mode_index = 2; mode_index <= 6; mode_index++)
                    for (int register_index = 0; register_index < 8;
                        ...
                        run_unary_case(unary_index, size_index, mode_index,
                                       register_index, 0);
                run_unary_case(unary_index, size_index, 7, 0, 0);
                run_unary_case(unary_index, size_index, 7, 1, 0);
            end
        end

        // NBCD has the same 50 M00 data-alterable EAs, but only byte size.
        // Four independent valid-BCD/flag classes distribute 0, 1, 50 and 99
        // across both X/Z inputs and both deterministic N/V values.
        for (int variant = 0; variant < 4; variant++) begin
            for (int register_index = 0; register_index < 8;
                 register_index++)
                run_unary_case(3, 0, 0, register_index, variant);
            for (int mode_index = 2; mode_index <= 6; mode_index++)
                for (int register_index = 0; register_index < 8;
                     register_index++)
                    run_unary_case(3, 0, mode_index, register_index,
                                   variant);
            run_unary_case(3, 0, 7, 0, variant);
            run_unary_case(3, 0, 7, 1, variant);
        end

        $display("PASS: all M00 CLR/NEG/NOT words and 50 NBCD words across four BCD/flag classes");
        $finish;
    end
endmodule
