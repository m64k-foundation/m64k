module m64k_core_binary_source_matrix_tb;
    import m64k_pkg::*;
    ...
    function automatic logic [7:0] operation_id(input integer operation,
        ...
        case (operation)
            0: return 8'(M64K_INSN_ADD_B + size_index);
            1: begin
                case (size_index)
                    0: return M64K_INSN_SUB_B;
                    1: return M64K_INSN_SUB_W;
                    default: return M64K_INSN_SUB_L;
                endcase
            end
            2: begin
                case (size_index)
                    0: return M64K_INSN_AND_B;
                    1: return M64K_INSN_AND_W;
                    default: return M64K_INSN_AND_L;
                endcase
            end
            3: return 8'(M64K_INSN_OR_B + size_index);
            default: begin
                case (size_index)
                    0: return M64K_INSN_CMP_B;
                    1: return M64K_INSN_CMP_W;
                    default: return M64K_INSN_CMP_L;
                endcase
            end
        endcase
    endfunction
    ...
    task automatic run_source_case(input integer operation,
                                   input integer size_index,
        ...
        begin
            ...
            compare_only = (operation == 4);
            arithmetic = (operation == 0) || (operation == 1) ||
                         compare_only;
            subtract = (operation == 1) || compare_only;
            mask = size_mask(size_index);
            sign_bit = sign_mask(size_index);
            sample_values(operation * 11 + size_index * 7 +
            ...
            else if ((ea_mode == 7) && (ea_register == 3))
            ...
            set_instruction_word(16'h102, destination[31:16]);
            set_instruction_word(16'h104, destination[15:0]);
            if (ea_mode == 0)
            ...
            opcode = {operation_nibble(operation),
                      ...
                      size_index[1:0], ea_mode[2:0], ea_register[2:0]};
            ...
            if ((ea_mode == 5)) begin
                ...
                set_instruction_word(cursor, 16'h010e); // $220 - $112
                cursor = cursor + 2;
            end else if ((ea_mode == 7) && (ea_register == 3)) begin
                set_instruction_word(cursor, 16'he06e); // A6.W + $6e
                cursor = cursor + 2;
            ...
            if (memory_source)
                store_operand(effective_address, size_index, source_sized);
            ...
            while (!(retire_valid && (retire_pc == 32'h0000_0110))) begin
                ...
                if (case_cycles > 500)
                    $fatal(1,
                        "binary source timeout: op=%0d size=%0d D=%0d ea=%0d/%0d",
            ...
            assert (debug_data_registers[destination_register*32 +: 32] ==
                    expected_register)
                else $fatal(1,
                    "binary source result: op=%0d size=%0d D=%0d ea=%0d/%0d opcode=%04x got=%08x expected=%08x source=%08x destination=%08x",
                    operation, size_index, destination_register, ea_mode,
                    ea_register, opcode,
                    debug_data_registers[destination_register*32 +: 32],
                    expected_register, source, destination);
            assert (debug_sr == {11'h138, expected_ccr})
                else $fatal(1,
                    "binary source SR: op=%0d size=%0d D=%0d ea=%0d/%0d opcode=%04x got=%04x expected=%04x",
                    operation, size_index, destination_register, ea_mode,
                    ea_register, opcode, debug_sr,
                    {11'h138, expected_ccr});
        ...
        for (int operation = 0; operation < 5; operation++) begin
            for (int size_index = 0; size_index < 3; size_index++) begin
                for (int destination_register = 0;
                    ...
                    for (int ea_register = 0; ea_register < 8; ea_register++)
                        run_source_case(operation, size_index,
                                        destination_register, 0, ea_register);
                    if ((size_index != 0) &&
                    ...
                    for (int ea_register = 0; ea_register <= 4; ea_register++)
                        run_source_case(operation, size_index,
                                        destination_register, 7, ea_register);
                end
            end
        end

        $display("PASS: all 6744 legal M00 <ea>,Dn binary words");
        $finish;
    end
endmodule
