module m64k_fx68k_smoke_tb;
    ...
    m64k_mem_if mem_bus(.clk(clk), .rst_n(!reset));
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            if (mem_bus.req_valid && mem_bus.req_ready &&
                mem_bus.req.command == M64K_MEM_READ &&
                mem_bus.req.instruction &&
                mem_bus.req.addr == 32'h0000_0102) begin
                loop_fetches <= loop_fetches + 1;
                if (loop_fetches == 2) begin
                    $display("PASS: fx68k executed reset vectors, NOP and BRA through M64K fabric");
                    $finish;
                end
            end

            if (cycle_count > 20000)
                $fatal(1, "fx68k did not reach the loop through M64K fabric");
        end
    end
