module m64k_router_tb;
    import m64k_pkg::*;

    logic clk;
    logic rst_n;
    m64k_mem_if upstream(.clk(clk), .rst_n(rst_n));
    m64k_mem_if port0(.clk(clk), .rst_n(rst_n));
    m64k_mem_if port1(.clk(clk), .rst_n(rst_n));
    m64k_mem_if port2(.clk(clk), .rst_n(rst_n));
    ...
    m64k_ram #(.BASE_ADDR(32'h0000_0000), .MEM_BYTES(256)) ram0 (
        .clk, .rst_n, .mem(port0)
    );
    m64k_ram #(.BASE_ADDR(32'h1000_0000), .MEM_BYTES(256)) ram1 (
        .clk, .rst_n, .mem(port1)
    );
    m64k_ram #(.BASE_ADDR(32'hffff_0000), .MEM_BYTES(256)) ram2 (
        .clk, .rst_n, .mem(port2)
    );
    ...
    task automatic transact(
        input m64k_mem_req_t request_value,
    ...
    initial begin
        ...
        request_value = '0;
        request_value.command = M64K_MEM_READ;
        request_value.size = M64K_SIZE_LONG;
        ...
        $display("PASS: M64K three-port address router and unmapped faults");
        $finish;
    end
endmodule
