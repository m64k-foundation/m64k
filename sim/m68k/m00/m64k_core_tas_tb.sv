    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (dmem_bus.req_valid && dmem_bus.req_ready &&
                (dmem_bus.req.command == M64K_MEM_ATOMIC)) begin
                assert (dmem_bus.req.atomic_op == M64K_ATOMIC_OR);
                assert (dmem_bus.req.size == M64K_SIZE_BYTE);
            ...
            if (cycles > 2000)
                $fatal(1, "M64K TAS test timed out");
        end
    end
