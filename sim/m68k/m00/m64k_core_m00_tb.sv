module m64k_core_m00_tb;
    ...
    m64k_ram #(.MEM_BYTES(512), .REQUEST_STALL_CYCLES(1)) instruction_ram (
        .clk, .rst_n, .mem(imem_bus)
    );
    m64k_ram #(.MEM_BYTES(512), .REQUEST_STALL_CYCLES(2)) vector_ram (
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (cycles > 3000)
                $fatal(1, "M64K native M00 core test timed out");
        end
    end
    ...
    initial begin
        ...
        assert (retired_pcs[0] == 32'h100 && retired_ids[0] == M64K_INSN_MOVEQ);
        assert (retired_pcs[1] == 32'h102 && retired_ids[1] == M64K_INSN_BCC);
        assert (retired_pcs[2] == 32'h106 && retired_ids[2] == M64K_INSN_MOVEQ);
        assert (retired_pcs[3] == 32'h108 &&
                retired_ids[3] == M64K_INSN_MOVE_B);
        assert (retired_pcs[4] == 32'h110 && retired_ids[4] == M64K_INSN_STOP);
        ...
        assert (terminal_exception.vector == M64K_VECTOR_ACCESS_FAULT);
        assert (terminal_exception.exception_class == M64K_EXC_DOUBLE_FAULT);
        assert (terminal_exception.stage == M64K_FAULT_STAGE_FRAME);
