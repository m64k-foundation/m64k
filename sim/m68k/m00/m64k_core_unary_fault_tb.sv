module m64k_core_unary_fault_tb;
    ...
    function automatic logic [15:0] unary_opcode(input integer unary_index);
        case (unary_index)
            ...
            2: return 16'h4618; // NOT.B (A0)+
            default: return 16'h4818; // NBCD (A0)+
        endcase
    endfunction
    ...
    initial begin
        rst_n = 1'b0;
        cycles = 0;
        for (int unary_index = 0; unary_index < 4; unary_index++) begin
            run_fault_case(unary_index, 1'b0);
            run_fault_case(unary_index, 1'b1);
        end

        $display("PASS: CLR/NEG/NOT/NBCD read/write faults suppress all RMW commit");
        $finish;
    end
endmodule
