module m64k_core_shift_tb;
    ...
    m64k_ram #(.MEM_BYTES(512)) data_ram (
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            cycles <= cycles + 1;
            if (cycles > 5000)
                $fatal(1, "M64K register shift test timed out");
        end
    end
    ...
    task automatic run_opcode_case(
        ...
        m64k_alu_result_t expected_shift;
        integer next_pc;
        integer shift_pc;
        integer case_cycles;
        begin
            ...
            expected_shift = reference_shift(
                operand_value, m64k_operand_size_t'(size_index),
            ...
            expected_sr[M64K_SR_X] = expected_shift.flags.x;
            expected_sr[M64K_SR_N] = expected_shift.flags.n;
            expected_sr[M64K_SR_Z] = expected_shift.flags.z;
            expected_sr[M64K_SR_V] = expected_shift.flags.v;
            expected_sr[M64K_SR_C] = expected_shift.flags.c;
