module m64k_uart_16550_model #(
    ...
    always_comb begin
        ...
        response_for_request.fault = request_in_range ? M64K_FAULT_NONE :
                                                        M64K_FAULT_ACCESS;
        byte_address = '0;
        read_byte = '0;

        if (request_in_range && (mem.req.command == M64K_MEM_READ)) begin
            for (int unsigned lane = 0; lane < M64K_LINE_BYTES;
                 lane = lane + 1) begin
                ...
                response_for_request.rdata[lane*8 +: 8] = read_byte;
                     ...
                     (mem.req.command != M64K_MEM_FENCE)) begin
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (mem.req_valid && mem.req_ready) begin
                ...
                if (request_in_range && (mem.req.command == M64K_MEM_WRITE)) begin
                    for (int unsigned lane = 0; lane < M64K_LINE_BYTES;
                         lane = lane + 1) begin
