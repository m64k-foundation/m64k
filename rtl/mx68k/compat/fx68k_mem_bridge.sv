module fx68k_mem_bridge #(
    ...
    mx_mem_req_t request_q;
    mx_mem_req_t request_from_bus;
    ...
    always_comb begin
        request_from_bus = '0;
        request_from_bus.command = rw_n ? MX_MEM_READ : MX_MEM_WRITE;
        request_from_bus.size = (!uds_n && !lds_n) ? MX_SIZE_WORD : MX_SIZE_BYTE;
        request_from_bus.atomic_op = MX_ATOMIC_NONE;
            ...
            case (state_q)
                ...
                BRIDGE_WAIT: begin
                    if (mem.rsp_valid && mem.rsp_ready) begin
                        ...
                        if (mem.rsp.fault == MX_FAULT_NONE)
                            state_q <= BRIDGE_ACK;
                        else
