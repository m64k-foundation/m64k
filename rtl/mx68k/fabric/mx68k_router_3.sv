module mx68k_router_3 #(
    ...
    input logic clk,
    input logic rst_n,
    mx68k_mem_if.slave upstream,
    mx68k_mem_if.master port0,
    mx68k_mem_if.master port1,
    mx68k_mem_if.master port2
);
    import mx68k_pkg::*;
    ...
    initial begin
        if (PORT0_ENABLE && ((PORT0_BASE & ~PORT0_MASK) != 32'd0))
            $fatal(1, "mx68k_router_3 PORT0_BASE is not canonical for its mask");
        if (PORT1_ENABLE && ((PORT1_BASE & ~PORT1_MASK) != 32'd0))
            $fatal(1, "mx68k_router_3 PORT1_BASE is not canonical for its mask");
        if (PORT2_ENABLE && ((PORT2_BASE & ~PORT2_MASK) != 32'd0))
            $fatal(1, "mx68k_router_3 PORT2_BASE is not canonical for its mask");
        if (PORT0_ENABLE && !mask_is_prefix(PORT0_MASK))
            $fatal(1, "mx68k_router_3 PORT0_MASK is not a contiguous prefix");
        if (PORT1_ENABLE && !mask_is_prefix(PORT1_MASK))
            $fatal(1, "mx68k_router_3 PORT1_MASK is not a contiguous prefix");
        if (PORT2_ENABLE && !mask_is_prefix(PORT2_MASK))
            $fatal(1, "mx68k_router_3 PORT2_MASK is not a contiguous prefix");
        if (PORT0_ENABLE && PORT1_ENABLE &&
            regions_overlap(PORT0_BASE, PORT0_MASK, PORT1_BASE, PORT1_MASK))
            $fatal(1, "mx68k_router_3 PORT0 and PORT1 overlap");
        if (PORT0_ENABLE && PORT2_ENABLE &&
            regions_overlap(PORT0_BASE, PORT0_MASK, PORT2_BASE, PORT2_MASK))
            $fatal(1, "mx68k_router_3 PORT0 and PORT2 overlap");
        if (PORT1_ENABLE && PORT2_ENABLE &&
            regions_overlap(PORT1_BASE, PORT1_MASK, PORT2_BASE, PORT2_MASK))
            $fatal(1, "mx68k_router_3 PORT1 and PORT2 overlap");
    end
                ...
                ROUTER_IDLE: begin
                    if (upstream.req_valid && upstream.req_ready) begin
                        if (match0)
                            ...
                            local_response_q.fault <= MX_FAULT_ACCESS;
