module mx68k_ram #(
    ...
    parameter mx68k_pkg::mx_mem_fault_t INJECT_FAULT_KIND =
        mx68k_pkg::MX_FAULT_ACCESS
) (
    input logic clk,
    input logic rst_n,
    ...
    wire [31:0] request_line = mx68k_line_base(mem.req.addr);
    ...
    wire inject_request_fault = INJECT_FAULT_ENABLE &&
        (((mem.req.command == MX_MEM_READ) && INJECT_FAULT_READ &&
          (mem.req.addr == INJECT_FAULT_READ_ADDR)) ||
         ((mem.req.command == MX_MEM_WRITE) && INJECT_FAULT_WRITE &&
    ...
    initial begin
        if ((MEM_BYTES == 0) || ((MEM_BYTES % MX68K_LINE_BYTES) != 0))
            $fatal(1, "mx68k_ram MEM_BYTES must be a non-zero multiple of 16");
        if (BASE_ADDR[3:0] != 4'b0000)
            $fatal(1, "mx68k_ram BASE_ADDR must be 16-byte aligned");
        if (CLEAR_ON_INIT)
        ...
        if (inject_request_fault) begin
            ...
            case (mem.req.command)
                ...
                MX_MEM_ATOMIC: begin
                    ...
                    if (mem.req.atomic_op == MX_ATOMIC_OR) begin
                        for (int unsigned read_lane = 0;
                             read_lane < MX68K_LINE_BYTES;
