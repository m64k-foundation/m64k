module m64k_core_m00 #(
    ...
    output m64k_arch_pkg::m64k_exception_t terminal_exception,
    ...
    m64k_irq_if.target irq,
    m64k_mem_if.master imem,
    m64k_mem_if.master dmem
);
    import m64k_pkg::*;
    import m64k_arch_pkg::*;
    import m64k_ea_pkg::*;
    import m64k_uop_pkg::*;
    import m64k_sequence_pkg::*;
    ...
    m64k_exception_t terminal_exception_q;
    logic redirect_pending_q;
    logic [31:0] redirect_target_q;
    m64k_uop_t pending_memory_uop_q;
    ...
    m64k_exception_t pending_exception_q;
    ...
    m64k_operand_size_t alu_operation_size;
    logic [31:0] data_alu_source_value;
    m64k_operand_size_t data_alu_operation_size;
    logic [31:0] direct_result_value;
    logic [5:0] shift_count;
    logic memory_shift_active;
    logic [31:0] shift_operand;
    m64k_operand_size_t shift_operation_size;
    logic [5:0] shift_operation_count;
    logic shift_direction_left;
    logic [1:0] shift_kind;
    m64k_alu_result_t shift_result;
    m64k_alu_result_t direct_add_extend_result;
    m64k_alu_result_t direct_subtract_extend_result;
    m64k_alu_result_t data_add_extend_result;
    m64k_alu_result_t data_subtract_extend_result;
    m64k_alu_result_t rmw_add_extend_result;
    m64k_alu_result_t rmw_subtract_extend_result;
    logic divider_start;
    logic divider_busy;
    logic divider_done;
    ...
    m64k_sequence_program_t sequence_program_q;
    m64k_sequence_step_t sequence_step_q;
    ...
    function automatic m64k_exception_t make_terminal_exception(
        input logic [7:0] vector,
        input m64k_exception_class_t exception_class,
        input m64k_fault_stage_t stage,
    ...
    function automatic m64k_exception_t make_data_exception(
        input m64k_mem_fault_t fault,
        input m64k_uop_t faulting_uop,
        ...
        begin
            value = make_terminal_exception(
                (fault == M64K_FAULT_ALIGNMENT) ? M64K_VECTOR_ADDRESS_ERROR :
                                                M64K_VECTOR_ACCESS_FAULT,
                M64K_EXC_DATA, M64K_FAULT_STAGE_MEMORY,
            ...
            value.function_code = m64k_m00_function_code(
                supervisor, 1'b0);
            value.special_status = m64k_m00_special_status_word(
    ...
    function automatic m64k_exception_t make_m00_fetch_exception(
        input m64k_exception_t original,
        input logic supervisor
    );
        m64k_exception_t value;
        begin
    ...
    function automatic m64k_exception_t make_execute_exception(
        input logic [7:0] vector,
        input m64k_uop_t faulting_uop
    );
        m64k_exception_t value;
        begin
            value = make_terminal_exception(
                vector, M64K_EXC_EXECUTE, M64K_FAULT_STAGE_EXECUTE,
                faulting_uop.instruction_pc, faulting_uop.sequential_pc);
            value.opcode = faulting_uop.instruction_word;
            value.operand_size = faulting_uop.size;
            value.rerunnable = 1'b0;
    ...
    function automatic m64k_exception_t make_interrupt_exception(
        input logic [7:0] vector,
        input logic [31:0] pc
    );
        m64k_exception_t value;
        begin
            value = make_terminal_exception(
                vector, M64K_EXC_INTERRUPT, M64K_FAULT_STAGE_COMMIT, pc, pc);
            value.instruction = 1'b0;
    ...
    function automatic m64k_exception_t make_instruction_trap_exception(
        input logic [7:0] vector,
        input m64k_uop_t trapping_uop
    );
        m64k_exception_t value;
        begin
            value = make_execute_exception(vector, trapping_uop);
            value.exception_class = M64K_EXC_TRAP;
            value.rerunnable = 1'b0;
            return value;
        end
    endfunction

    function automatic m64k_exception_t make_trace_exception(
        input logic [31:0] pc
    );
        m64k_exception_t value;
        begin
            value = make_terminal_exception(
                M64K_VECTOR_TRACE, M64K_EXC_TRACE, M64K_FAULT_STAGE_COMMIT, pc, pc);
            value.instruction = 1'b0;
            value.rerunnable = 1'b1;
            return value;
        end
    ...
    function automatic m64k_exception_t make_frame_exception(
        input m64k_exception_t original,
        input logic [31:0] fault_address
    );
        m64k_exception_t value;
        begin
            value = original;
            value.valid = 1'b1;
            value.exception_class = M64K_EXC_DOUBLE_FAULT;
            value.stage = M64K_FAULT_STAGE_FRAME;
            value.logical_address = fault_address;
            value.physical_address = fault_address;
            value.rerunnable = 1'b0;
            return value;
    ...
    m64k_register_file registers (
        ...
        .data_read_index_a(
            (instruction_uop.source_a.kind == M64K_OPERAND_DATA_REGISTER) ?
        ...
        .address_read_index_a(
            (instruction_uop.source_a.kind == M64K_OPERAND_ADDRESS_REGISTER) ?
            instruction_uop.source_a.index[2:0] :
            instruction_uop.source_ea.register_index),
    ...
    function automatic m64k_mem_size_t memory_size_for_operand(
        input m64k_operand_size_t size
    );
        case (size)
            M64K_OP_BYTE: return M64K_SIZE_BYTE;
            M64K_OP_WORD: return M64K_SIZE_WORD;
            default: return M64K_SIZE_LONG;
        endcase
    endfunction
    ...
    function automatic logic [31:0] merge_data_register(
        ...
        case (size)
            M64K_OP_BYTE: return {original[31:8], value[7:0]};
            M64K_OP_WORD: return {original[31:16], value[15:0]};
    ...
    function automatic logic [15:0] chk_word_result_sr(
        ...
        begin
            ...
            bound = upper_bound[15:0];
            if (checked < 16'sd0)
                value[M64K_SR_N] = 1'b1;
            else if (checked > bound)
                value[M64K_SR_N] = 1'b0;
    ...
    function automatic logic [15:0] logical_result_sr(
        ...
        begin
            ...
            case (size)
                M64K_OP_BYTE: begin
                    value[M64K_SR_N] = result[7];
                    value[M64K_SR_Z] = (result[7:0] == 8'd0);
                end
                M64K_OP_WORD: begin
                    value[M64K_SR_N] = result[15];
                    value[M64K_SR_Z] = (result[15:0] == 16'd0);
                end
                default: begin
                    value[M64K_SR_N] = result[31];
                    value[M64K_SR_Z] = (result == 32'd0);
                end
            endcase
            value[M64K_SR_V] = 1'b0;
            value[M64K_SR_C] = 1'b0;
            return value;
        end
    endfunction
    ...
    function automatic logic [15:0] add_result_sr(
        ...
        begin
            ...
            case (size)
                M64K_OP_BYTE: begin
                    value[M64K_SR_N] = sum_byte[7];
                    value[M64K_SR_Z] = (sum_byte[7:0] == 8'd0);
                    value[M64K_SR_C] = sum_byte[8];
                    sign_destination = destination_value[7];
                    sign_source = source_value[7];
                    sign_result = sum_byte[7];
                end
                M64K_OP_WORD: begin
                    value[M64K_SR_N] = sum_word[15];
                    value[M64K_SR_Z] = (sum_word[15:0] == 16'd0);
                    value[M64K_SR_C] = sum_word[16];
                    sign_destination = destination_value[15];
                    sign_source = source_value[15];
                    sign_result = sum_word[15];
                end
                default: begin
                    value[M64K_SR_N] = sum_long[31];
                    value[M64K_SR_Z] = (sum_long[31:0] == 32'd0);
                    value[M64K_SR_C] = sum_long[32];
                    sign_destination = destination_value[31];
                    sign_source = source_value[31];
                    sign_result = sum_long[31];
                end
            endcase
            value[M64K_SR_V] = !(sign_destination ^ sign_source) &&
                             (sign_result ^ sign_destination);
            value[M64K_SR_X] = value[M64K_SR_C];
            return value;
        end
    endfunction
        ...
        begin
            ...
            case (size)
                M64K_OP_BYTE: begin
                    value[M64K_SR_N] = result[7];
                    value[M64K_SR_Z] = (result[7:0] == 8'd0);
                    value[M64K_SR_C] = source_value[7:0] >
                ...
                M64K_OP_WORD: begin
                    value[M64K_SR_N] = result[15];
                    value[M64K_SR_Z] = (result[15:0] == 16'd0);
                    value[M64K_SR_C] = source_value[15:0] >
                ...
                default: begin
                    value[M64K_SR_N] = result[31];
                    value[M64K_SR_Z] = (result == 32'd0);
                    value[M64K_SR_C] = source_value > destination_value;
                    sign_destination = destination_value[31];
                    sign_source = source_value[31];
    ...
    function automatic logic [15:0] extend_arithmetic_result_sr(
        input logic [15:0] old_sr,
        input m64k_alu_result_t result
    );
        logic [15:0] value;
        begin
            value = old_sr;
            value[M64K_SR_X] = result.flags.x;
            value[M64K_SR_N] = result.flags.n;
            value[M64K_SR_Z] = result.flags.z;
            value[M64K_SR_V] = result.flags.v;
            value[M64K_SR_C] = result.flags.c;
            return value;
        end
    endfunction
    ...
    always_comb begin
        reset_request = '0;
        reset_request.command = M64K_MEM_READ;
        reset_request.size = M64K_SIZE_LINE;
        ...
        pending_operand_bytes = (pending_memory_action_q inside {
            ...
            ((pending_memory_uop_q.size == M64K_OP_BYTE) ? 5'd1 :
             (pending_memory_uop_q.size == M64K_OP_WORD) ? 5'd2 : 5'd4));
        ...
        data_request.command =
            (pending_memory_action_q == MEMORY_ACTION_ATOMIC) ?
                M64K_MEM_ATOMIC :
            pending_request_is_write ? M64K_MEM_WRITE : M64K_MEM_READ;
        data_request.size = (data_request_splits_long ||
                             split_access_second_q) ? M64K_SIZE_WORD :
            ((pending_memory_action_q inside {
            MEMORY_ACTION_JSR_PUSH, MEMORY_ACTION_RTS_POP,
            MEMORY_ACTION_PEA_PUSH, MEMORY_ACTION_EXCEPTION_PUSH_PC,
            MEMORY_ACTION_EXCEPTION_PUSH_FAULT_ADDRESS,
            MEMORY_ACTION_EXCEPTION_VECTOR_READ,
            MEMORY_ACTION_RTE_LOAD_PC,
            MEMORY_ACTION_RTR_LOAD_PC}) ? M64K_SIZE_LONG :
            ((pending_memory_action_q inside {
              MEMORY_ACTION_EXCEPTION_PUSH_SR,
              MEMORY_ACTION_EXCEPTION_PUSH_IR,
              MEMORY_ACTION_EXCEPTION_PUSH_SSW,
              MEMORY_ACTION_RTE_LOAD_SR,
              MEMORY_ACTION_RTR_LOAD_CCR}) ? M64K_SIZE_WORD :
        ...
        data_request.supervisor = debug_sr[M64K_SR_S] ||
            (pending_memory_action_q inside {
             MEMORY_ACTION_EXCEPTION_PUSH_PC,
             MEMORY_ACTION_EXCEPTION_PUSH_SR,
             MEMORY_ACTION_EXCEPTION_PUSH_IR,
        ...
        data_request_alignment_error =
            (data_request.size inside {M64K_SIZE_WORD, M64K_SIZE_LONG}) &&
            data_request.addr[0];
        ...
        if (data_response_ids_match &&
            (pending_memory_action_q == MEMORY_ACTION_ATOMIC) &&
            (dmem.rsp.fault == M64K_FAULT_NONE) &&
            !dmem.rsp.atomic_success)
            data_response_fault = M64K_FAULT_UNSUPPORTED;
        ...
        pending_exception_special = pending_exception_q.vector inside {
            M64K_VECTOR_ACCESS_FAULT, M64K_VECTOR_ADDRESS_ERROR};
        ...
        movem_load_write_valid = data_response_completes_access &&
            (data_response_fault == M64K_FAULT_NONE) &&
        ...
        condition_flags.n = debug_sr[M64K_SR_N];
        condition_flags.z = debug_sr[M64K_SR_Z];
        condition_flags.v = debug_sr[M64K_SR_V];
        condition_flags.c = debug_sr[M64K_SR_C];
        branch_taken = (instruction_uop.condition == 4'h0) ||
                       ((instruction_uop.condition >= 4'h2) &&
                        m64k_condition_true(instruction_uop.condition,
                                          condition_flags));
        trap_taken = (instruction_uop.opcode == M64K_UOP_TRAP) &&
            ((instruction_uop.flags_read == '0) ||
             m64k_condition_true(instruction_uop.condition, condition_flags));
        dbcc_condition_false = !m64k_condition_true(
        ...
        if (instruction_uop.source_a.kind == M64K_OPERAND_SR)
            source_operand_value = {16'd0, debug_sr};
        else if (instruction_uop.source_a.kind == M64K_OPERAND_USP)
            source_operand_value = debug_usp;
        else if ((instruction_uop.source_ea.kind == M64K_EA_DATA_REGISTER) ||
            (instruction_uop.source_a.kind == M64K_OPERAND_DATA_REGISTER))
            source_operand_value = rf_data_read_a;
        else if ((instruction_uop.source_ea.kind == M64K_EA_ADDRESS_REGISTER) ||
                 (instruction_uop.source_a.kind == M64K_OPERAND_ADDRESS_REGISTER))
            source_operand_value = rf_address_read_a;

        destination_operand_value = rf_data_read_b;
        if (instruction_uop.destination.kind == M64K_OPERAND_ADDRESS_REGISTER)
            destination_operand_value = rf_address_read_b;
        else if (instruction_uop.destination.kind == M64K_OPERAND_IMMEDIATE)
        ...
        memory_shift_active = (pending_memory_uop_q.opcode == M64K_UOP_SHIFT) &&
        ...
        direct_add_extend_result = instruction_uop.condition[1] ?
            m64k_add_decimal_byte(destination_operand_value[7:0],
                                source_operand_value[7:0],
                                debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            m64k_add(destination_operand_value, source_operand_value,
                   instruction_uop.size, debug_sr[M64K_SR_X],
                   debug_sr[M64K_SR_Z]);
        direct_subtract_extend_result = instruction_uop.condition[1] ?
            m64k_subtract_decimal_byte(
                                     instruction_uop.condition[0] ?
                                     ...
                                     debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            instruction_uop.condition[0] ?
            m64k_subtract(destination_operand_value, source_operand_value,
                        instruction_uop.size, debug_sr[M64K_SR_X],
                        debug_sr[M64K_SR_Z]) :
            m64k_subtract(32'd0, destination_operand_value,
                        instruction_uop.size, debug_sr[M64K_SR_X],
                        debug_sr[M64K_SR_Z]);
        data_add_extend_result = pending_memory_uop_q.condition[1] ?
            m64k_add_decimal_byte(pending_destination_original_q[7:0],
                                pending_source_operand_q[7:0],
                                debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            m64k_add(pending_destination_original_q, pending_source_operand_q,
        ...
        data_subtract_extend_result = pending_memory_uop_q.condition[1] ?
            m64k_subtract_decimal_byte(
                                     ...
                                     debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            pending_memory_uop_q.condition[0] ?
            m64k_subtract(pending_destination_original_q,
                        pending_source_operand_q,
                        pending_memory_uop_q.size,
                        debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            m64k_subtract(32'd0, pending_destination_original_q,
                        pending_memory_uop_q.size,
                        debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]);
        rmw_add_extend_result = pending_memory_uop_q.condition[1] ?
            m64k_add_decimal_byte(data_response_value[7:0],
                                pending_source_operand_q[7:0],
                                debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            m64k_add(data_response_value, pending_source_operand_q,
                   pending_memory_uop_q.size,
                   debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]);
        rmw_subtract_extend_result = pending_memory_uop_q.condition[1] ?
            m64k_subtract_decimal_byte(
                                     ...
                                     debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            pending_memory_uop_q.condition[0] ?
            m64k_subtract(data_response_value, pending_source_operand_q,
                        pending_memory_uop_q.size,
                        debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]) :
            m64k_subtract(32'd0, data_response_value,
                        pending_memory_uop_q.size,
                        debug_sr[M64K_SR_X], debug_sr[M64K_SR_Z]);
        alu_source_value = source_operand_value;
        alu_operation_size = instruction_uop.size;
        if ((instruction_uop.opcode inside {
             M64K_UOP_ADD, M64K_UOP_SUBTRACT, M64K_UOP_COMPARE}) &&
            (instruction_uop.destination.kind ==
             M64K_OPERAND_ADDRESS_REGISTER)) begin
            if (instruction_uop.size == M64K_OP_WORD)
                alu_source_value = {{16{source_operand_value[15]}},
                                    source_operand_value[15:0]};
            alu_operation_size = M64K_OP_LONG;
        end

        source_ea_address = m64k_ea_address(
            ...
            packed_register_value(debug_address_registers,
                instruction_uop.source_ea.extension_0[14:12]));
        destination_ea_address = m64k_ea_address(
            ...
            packed_register_value(debug_address_registers,
                instruction_uop.destination_ea.extension_0[14:12]));
        sequence_source_step = {29'd0, m64k_ea_step_bytes(
            instruction_uop.size,
            instruction_uop.source_ea.register_index)};
        sequence_destination_step = {29'd0, m64k_ea_step_bytes(
            instruction_uop.size,
            instruction_uop.destination_ea.register_index)};

        starts_memory_sequence =
            (instruction_uop.opcode == M64K_UOP_COMPARE_MEMORY) ||
            ((instruction_uop.opcode == M64K_UOP_MOVE) &&
             m64k_ea_is_memory(instruction_uop.destination_ea) &&
             !m64k_ea_is_memory(instruction_uop.source_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_MOVE) &&
             m64k_ea_is_memory(instruction_uop.destination_ea) &&
             m64k_ea_is_memory(instruction_uop.source_ea)) ||
            (((instruction_uop.opcode == M64K_UOP_MOVE) ||
              (instruction_uop.opcode == M64K_UOP_MOVE_ADDRESS)) &&
             m64k_ea_is_memory(instruction_uop.source_ea) &&
             !m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            ((instruction_uop.opcode inside {
              M64K_UOP_ADD, M64K_UOP_SUBTRACT, M64K_UOP_AND, M64K_UOP_OR,
              M64K_UOP_XOR, M64K_UOP_COMPARE, M64K_UOP_MULTIPLY,
              M64K_UOP_DIVIDE, M64K_UOP_CHECK_BOUNDS}) &&
             m64k_ea_is_memory(instruction_uop.source_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_CLEAR) &&
             m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_SET_CONDITION) &&
             m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_ATOMIC) &&
             m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_TEST) &&
             m64k_ea_is_memory(instruction_uop.source_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_BIT_TEST) &&
             m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_SHIFT) &&
             m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            ((instruction_uop.opcode == M64K_UOP_COMPARE) &&
             m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            ((instruction_uop.opcode inside {
              M64K_UOP_ADD, M64K_UOP_SUBTRACT, M64K_UOP_AND, M64K_UOP_OR,
              M64K_UOP_XOR, M64K_UOP_NOT, M64K_UOP_NEGATE,
              M64K_UOP_ADD_EXTEND, M64K_UOP_SUBTRACT_EXTEND}) &&
             m64k_ea_is_memory(instruction_uop.destination_ea)) ||
            (instruction_uop.opcode inside {
             M64K_UOP_JUMP_SUBROUTINE, M64K_UOP_BRANCH_SUBROUTINE}) ||
            (instruction_uop.opcode == M64K_UOP_RETURN) ||
            (instruction_uop.opcode == M64K_UOP_PUSH_EFFECTIVE_ADDRESS) ||
            ...
            ((instruction_uop.opcode == M64K_UOP_MOVE_MULTIPLE) &&
             (instruction_uop.immediate[15:0] != 16'd0)) ||
            (instruction_uop.opcode == M64K_UOP_LINK) ||
            (instruction_uop.opcode == M64K_UOP_UNLINK) ||
            (instruction_uop.opcode == M64K_UOP_EXCEPTION_RETURN) ||
            (instruction_uop.opcode == M64K_UOP_STORE);
        ...
        if (instruction_uop.valid) begin
            case (instruction_uop.opcode)
                ...
                M64K_UOP_ATOMIC,
                M64K_UOP_STOP: execute_supported = 1'b1;
        ...
        interrupt_take = !trace_pending_q &&
            ...
            ((interrupt_level7_pending_q || interrupt_level7_rise) ||
             (irq.request && m64k_interrupt_accepted(
                 irq.level, debug_sr[M64K_SR_I2:M64K_SR_I0], 1'b0)));
        accepted_interrupt_vector = irq.vector_valid ? irq.vector :
            (M64K_VECTOR_AUTOVECTOR_BASE + {5'd0, active_interrupt_level});
        ...
        sequence_checkpoint_commit_valid =
            ...
            (sequence_step_q == M64K_SEQUENCE_READ_SOURCE);
        direct_chk_trap =
            (instruction_uop.opcode == M64K_UOP_CHECK_BOUNDS) &&
            !m64k_ea_is_memory(instruction_uop.source_ea) &&
            chk_word_out_of_bounds(destination_operand_value,
                                   source_operand_value);
        data_chk_trap =
            (pending_memory_uop_q.opcode == M64K_UOP_CHECK_BOUNDS) &&
        ...
        if (instruction_uop.opcode == M64K_UOP_CLEAR)
            direct_result_value = 32'd0;
        if ((instruction_uop.opcode == M64K_UOP_MOVE_ADDRESS) &&
            (instruction_uop.size == M64K_OP_WORD))
            direct_result_value = {{16{source_operand_value[15]}},
                                   source_operand_value[15:0]};
        if (instruction_uop.opcode == M64K_UOP_CALCULATE_EA)
            direct_result_value = source_ea_address;
        if (instruction_uop.opcode == M64K_UOP_ADD)
            direct_result_value = destination_operand_value +
                                  alu_source_value;
        if (instruction_uop.opcode == M64K_UOP_ADD_EXTEND)
            direct_result_value = direct_add_extend_result.result;
        if (instruction_uop.opcode == M64K_UOP_SUBTRACT)
            direct_result_value = destination_operand_value -
                                  alu_source_value;
        if (instruction_uop.opcode == M64K_UOP_COMPARE)
            direct_result_value = destination_operand_value -
                                  alu_source_value;
        if (instruction_uop.opcode == M64K_UOP_AND)
            direct_result_value = destination_operand_value &
                                  source_operand_value;
        if (instruction_uop.opcode == M64K_UOP_OR)
            direct_result_value = destination_operand_value |
                                  source_operand_value;
        if (instruction_uop.opcode == M64K_UOP_XOR)
            direct_result_value = destination_operand_value ^
                                  source_operand_value;
        if (instruction_uop.opcode == M64K_UOP_NOT)
            direct_result_value = ~destination_operand_value;
        if (instruction_uop.opcode == M64K_UOP_NEGATE)
            direct_result_value = 32'd0 - destination_operand_value;
        if (instruction_uop.opcode == M64K_UOP_SUBTRACT_EXTEND)
            direct_result_value = direct_subtract_extend_result.result;
        if (instruction_uop.opcode == M64K_UOP_SWAP)
            direct_result_value = {destination_operand_value[15:0],
                                   destination_operand_value[31:16]};
        if (instruction_uop.opcode == M64K_UOP_SIGN_EXTEND)
        ...
        direct_writes_sr = direct_writes_flags ||
            (instruction_uop.destination.kind == M64K_OPERAND_SR) ||
            (instruction_uop.opcode == M64K_UOP_STOP);
        ...
        if (instruction_uop.opcode == M64K_UOP_SUBTRACT)
            direct_sr_value = subtract_result_sr(
                                                 ...
                                                 source_operand_value,
                                                 instruction_uop.size);
        if (instruction_uop.opcode == M64K_UOP_COMPARE) begin
            direct_sr_value = subtract_result_sr(
                debug_sr, destination_operand_value, alu_source_value,
                alu_operation_size);
            direct_sr_value[M64K_SR_X] = debug_sr[M64K_SR_X];
        end
        if (instruction_uop.opcode inside {
            M64K_UOP_AND, M64K_UOP_OR, M64K_UOP_XOR, M64K_UOP_NOT})
        ...
        if (instruction_uop.opcode == M64K_UOP_SUBTRACT_EXTEND)
            direct_sr_value = instruction_uop.condition[1] ?
                ...
                extend_arithmetic_result_sr(
                    debug_sr, direct_subtract_extend_result);
        if (instruction_uop.opcode == M64K_UOP_SWAP) begin
            direct_sr_value = logical_result_sr(debug_sr,
                                                direct_result_value,
                                                M64K_OP_LONG);
            direct_sr_value[M64K_SR_X] = debug_sr[M64K_SR_X];
        end
        if (instruction_uop.opcode == M64K_UOP_SIGN_EXTEND) begin
            direct_sr_value = logical_result_sr(debug_sr,
                                                direct_result_value,
                                                instruction_uop.size);
            direct_sr_value[M64K_SR_X] = debug_sr[M64K_SR_X];
        ...
        if (instruction_uop.opcode == M64K_UOP_BIT_TEST) begin
            ...
            direct_sr_value[M64K_SR_Z] =
                !destination_operand_value[direct_bit_index];
        end
        if (instruction_uop.opcode == M64K_UOP_ATOMIC) begin
            ...
            direct_sr_value = logical_result_sr(
                debug_sr, destination_operand_value, M64K_OP_BYTE);
        end
        if (instruction_uop.opcode == M64K_UOP_MULTIPLY) begin
            direct_sr_value = logical_result_sr(debug_sr,
                                                direct_result_value,
                                                M64K_OP_LONG);
            direct_sr_value[M64K_SR_X] = debug_sr[M64K_SR_X];
        end
        if (instruction_uop.opcode == M64K_UOP_SHIFT) begin
            direct_sr_value = debug_sr;
            direct_sr_value[M64K_SR_X] = shift_result.flags.x;
            direct_sr_value[M64K_SR_N] = shift_result.flags.n;
            direct_sr_value[M64K_SR_Z] = shift_result.flags.z;
            direct_sr_value[M64K_SR_V] = shift_result.flags.v;
            direct_sr_value[M64K_SR_C] = shift_result.flags.c;
        end
        if (instruction_uop.opcode == M64K_UOP_CHECK_BOUNDS)
            direct_sr_value = direct_chk_sr;
        if (instruction_uop.destination.kind == M64K_OPERAND_SR) begin
            if (instruction_uop.opcode == M64K_UOP_MOVE)
                direct_sr_value = instruction_uop.condition[0] ?
                    {debug_sr[15:5], source_operand_value[4:0]} :
                    source_operand_value[15:0];
            else if (instruction_uop.opcode == M64K_UOP_OR)
                direct_sr_value = debug_sr | instruction_uop.immediate[15:0];
            else if (instruction_uop.opcode == M64K_UOP_XOR)
                direct_sr_value = debug_sr ^ instruction_uop.immediate[15:0];
            else
                direct_sr_value = debug_sr & instruction_uop.immediate[15:0];
            if ((instruction_uop.opcode inside {
                 M64K_UOP_OR, M64K_UOP_AND, M64K_UOP_XOR}) &&
                instruction_uop.condition[0])
                direct_sr_value[15:8] = debug_sr[15:8];
        end else if (instruction_uop.opcode == M64K_UOP_STOP) begin
        ...
        if ((pending_memory_uop_q.opcode inside {
            ...
            (pending_memory_uop_q.destination.kind ==
             M64K_OPERAND_ADDRESS_REGISTER)) begin
            if (pending_memory_uop_q.size == M64K_OP_WORD)
                data_alu_source_value = {{16{data_response_value[15]}},
                                         data_response_value[15:0]};
            data_alu_operation_size = M64K_OP_LONG;
        end
        if (pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) begin
            case (pending_memory_uop_q.opcode)
                M64K_UOP_ADD: data_writeback_value =
                    pending_destination_original_q + data_alu_source_value;
                M64K_UOP_SUBTRACT: data_writeback_value =
                    pending_destination_original_q - data_alu_source_value;
                M64K_UOP_COMPARE: data_writeback_value =
                    m64k_ea_is_memory(pending_memory_uop_q.destination_ea) ?
                    (data_response_value - pending_source_operand_q) :
                    (pending_destination_original_q - data_alu_source_value);
                M64K_UOP_AND: data_writeback_value =
                    pending_destination_original_q & data_response_value;
                M64K_UOP_OR: data_writeback_value =
                    pending_destination_original_q | data_response_value;
                M64K_UOP_XOR: data_writeback_value =
                    pending_destination_original_q ^ data_response_value;
                M64K_UOP_MULTIPLY: data_writeback_value =
                    execute_multiply_word(
                        pending_destination_original_q,
                        data_response_value,
                        pending_memory_uop_q.condition[0]);
                M64K_UOP_DIVIDE: data_writeback_value =
                    divider_result;
        ...
        case (pending_memory_uop_q.opcode)
            M64K_UOP_ADD: rmw_result_value = data_response_value +
                                                pending_source_operand_q;
            M64K_UOP_SUBTRACT: rmw_result_value = data_response_value -
                                                pending_source_operand_q;
            M64K_UOP_AND: rmw_result_value = data_response_value &
                                                pending_source_operand_q;
            M64K_UOP_OR: rmw_result_value = data_response_value |
                                                pending_source_operand_q;
            M64K_UOP_XOR: rmw_result_value = data_response_value ^
                                                pending_source_operand_q;
            M64K_UOP_NOT: rmw_result_value = ~data_response_value;
            M64K_UOP_NEGATE: rmw_result_value = 32'd0 - data_response_value;
            M64K_UOP_ADD_EXTEND:
        ...
        data_sr_value = (pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) ?
            ((pending_memory_uop_q.opcode == M64K_UOP_ADD) ?
             ...
             (pending_memory_uop_q.opcode inside {
              M64K_UOP_SUBTRACT, M64K_UOP_COMPARE}) ?
        ...
        if ((pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) &&
            (pending_memory_uop_q.opcode == M64K_UOP_COMPARE) &&
            m64k_ea_is_memory(pending_memory_uop_q.destination_ea))
        ...
        if ((pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) &&
            (pending_memory_uop_q.opcode == M64K_UOP_COMPARE))
            data_sr_value[M64K_SR_X] = debug_sr[M64K_SR_X];
        if ((pending_memory_action_q == MEMORY_ACTION_SEQUENCE_READ) &&
            (sequence_program_q == M64K_SEQUENCE_CMPM) &&
            (sequence_step_q == M64K_SEQUENCE_READ_DESTINATION)) begin
            data_sr_value = subtract_result_sr(
                debug_sr, data_response_value, pending_source_operand_q,
                pending_memory_uop_q.size);
            data_sr_value[M64K_SR_X] = debug_sr[M64K_SR_X];
        ...
        if ((pending_memory_action_q == MEMORY_ACTION_MOVE_LOAD) &&
            (pending_memory_uop_q.opcode == M64K_UOP_MOVE) &&
            (pending_memory_uop_q.destination.kind == M64K_OPERAND_SR))
        ...
        if (pending_memory_action_q == MEMORY_ACTION_RMW_STORE) begin
            if (pending_memory_uop_q.opcode == M64K_UOP_ADD)
                ...
                data_sr_value = pending_memory_uop_q.condition[1] ?
                    ...
                    extend_arithmetic_result_sr(
                        debug_sr, data_subtract_extend_result);
            else if (pending_memory_uop_q.opcode == M64K_UOP_BIT_TEST) begin
                data_sr_value = debug_sr;
                data_sr_value[M64K_SR_Z] =
                    !pending_destination_original_q[memory_bit_index];
            end
            else if (pending_memory_uop_q.opcode == M64K_UOP_SHIFT) begin
                data_sr_value = debug_sr;
                data_sr_value[M64K_SR_X] = shift_result.flags.x;
                data_sr_value[M64K_SR_N] = shift_result.flags.n;
                data_sr_value[M64K_SR_Z] = shift_result.flags.z;
                data_sr_value[M64K_SR_V] = shift_result.flags.v;
                data_sr_value[M64K_SR_C] = shift_result.flags.c;
            end
            else
        ...
        if (pending_memory_action_q == MEMORY_ACTION_ATOMIC)
            data_sr_value = logical_result_sr(
                debug_sr, data_response_value, M64K_OP_BYTE);
        if ((pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) &&
            (pending_memory_uop_q.opcode == M64K_UOP_MULTIPLY)) begin
        ...
        if ((pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) &&
            (pending_memory_uop_q.opcode == M64K_UOP_DIVIDE)) begin
            data_sr_value = debug_sr;
            data_sr_value[M64K_SR_C] = 1'b0;
            data_sr_value[M64K_SR_V] = divider_overflow;
            if (!divider_overflow) begin
                data_sr_value[M64K_SR_N] = divider_n;
                data_sr_value[M64K_SR_Z] = divider_z;
            end
        end
        if ((pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) &&
            (pending_memory_uop_q.opcode == M64K_UOP_CHECK_BOUNDS))
        ...
        direct_commit_valid = instruction_handshake && successful_uop &&
                              !starts_memory_sequence &&
                              (instruction_uop.opcode != M64K_UOP_RESET) &&
                              (instruction_uop.opcode != M64K_UOP_DIVIDE) &&
                              !trap_taken &&
                              !direct_chk_trap;
        ...
        rf_boot_valid = reset_response_handshake &&
                        (reset_response_fault == M64K_FAULT_NONE);
        ...
        rf_data_write_enable =
            ...
            (data_commit_valid &&
             (pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) &&
             !(pending_memory_uop_q.opcode inside {
               M64K_UOP_COMPARE, M64K_UOP_CHECK_BOUNDS}) &&
             !((pending_memory_uop_q.opcode == M64K_UOP_DIVIDE) &&
               divider_overflow) &&
             (pending_memory_uop_q.destination.kind ==
              M64K_OPERAND_DATA_REGISTER)) ||
            ...
            (direct_commit_valid &&
             ...
             (instruction_uop.destination.kind ==
              M64K_OPERAND_DATA_REGISTER)) ||
            (direct_commit_valid &&
             (instruction_uop.opcode == M64K_UOP_EXCHANGE) &&
             (instruction_uop.destination.kind ==
              M64K_OPERAND_DATA_REGISTER)) ||
            (direct_commit_valid &&
             (instruction_uop.opcode inside {
              M64K_UOP_ADD, M64K_UOP_ADD_EXTEND, M64K_UOP_SUBTRACT,
              M64K_UOP_AND, M64K_UOP_OR,
              M64K_UOP_XOR, M64K_UOP_NOT, M64K_UOP_NEGATE, M64K_UOP_SWAP,
              M64K_UOP_SUBTRACT_EXTEND,
              M64K_UOP_SIGN_EXTEND, M64K_UOP_DBCC, M64K_UOP_SHIFT,
              M64K_UOP_MULTIPLY, M64K_UOP_DIVIDE}) &&
             ((instruction_uop.opcode != M64K_UOP_DBCC) ||
        ...
        rf_data_write_value = movem_load_write_valid ?
            ((pending_memory_uop_q.size == M64K_OP_WORD) ?
        ...
        rf_data_write_enable_b = direct_commit_valid &&
            (instruction_uop.opcode == M64K_UOP_EXCHANGE) &&
            (instruction_uop.source_a.kind == M64K_OPERAND_DATA_REGISTER) &&
            (instruction_uop.destination.kind == M64K_OPERAND_DATA_REGISTER);
        ...
        rf_address_write_enable =
            ...
            (direct_commit_valid &&
             ...
             (instruction_uop.destination.kind ==
              M64K_OPERAND_ADDRESS_REGISTER)) ||
            (direct_commit_valid &&
             (instruction_uop.opcode == M64K_UOP_CALCULATE_EA) &&
             (instruction_uop.destination.kind ==
              M64K_OPERAND_ADDRESS_REGISTER)) ||
            (direct_commit_valid &&
             (instruction_uop.opcode inside {M64K_UOP_ADD, M64K_UOP_SUBTRACT}) &&
             (instruction_uop.destination.kind ==
              M64K_OPERAND_ADDRESS_REGISTER)) ||
            (data_commit_valid &&
             (pending_memory_action_q == MEMORY_ACTION_MOVE_LOAD) &&
             (pending_memory_uop_q.opcode == M64K_UOP_MOVE_ADDRESS)) ||
            (data_commit_valid &&
             (pending_memory_action_q == MEMORY_ACTION_ALU_LOAD) &&
             (pending_memory_uop_q.opcode inside {
        ...
        rf_address_write_value =
            ...
            ((data_commit_valid &&
              ...
              (pending_memory_uop_q.destination.kind ==
               M64K_OPERAND_ADDRESS_REGISTER) &&
              (pending_memory_uop_q.opcode inside {
               M64K_UOP_ADD, M64K_UOP_SUBTRACT})) ? data_writeback_value :
            (data_commit_valid &&
             (pending_memory_action_q == MEMORY_ACTION_MOVE_LOAD) &&
             (pending_memory_uop_q.opcode == M64K_UOP_MOVE_ADDRESS)) ?
            ((pending_memory_uop_q.size == M64K_OP_WORD) ?
        ...
        rf_usp_write_enable = direct_commit_valid &&
            (instruction_uop.opcode == M64K_UOP_MOVE) &&
            (instruction_uop.destination.kind == M64K_OPERAND_USP);
        ...
        if (pending_exception_q.exception_class == M64K_EXC_INTERRUPT)
            exception_entry_sr[M64K_SR_I2:M64K_SR_I0] =
        ...
        if (direct_commit_valid &&
            (instruction_uop.opcode == M64K_UOP_BRANCH) && branch_taken)
            rf_pc_write_value = instruction_uop.immediate;
        if (direct_commit_valid &&
            (instruction_uop.opcode == M64K_UOP_DBCC) && dbcc_taken)
            rf_pc_write_value = instruction_uop.immediate;
        if (direct_commit_valid &&
            (instruction_uop.opcode == M64K_UOP_JUMP))
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            sequence_program_q <= M64K_SEQUENCE_NONE;
            sequence_step_q <= M64K_SEQUENCE_IDLE;
            ...
            case (state_q)
                ...
                CORE_RESET_WAIT: begin
                    if (reset_response_handshake) begin
                        if (reset_response_fault == M64K_FAULT_NONE) begin
                            state_q <= CORE_START_FRONTEND;
                        end else begin
                            terminal_exception_q <= make_terminal_exception(
                                M64K_VECTOR_ACCESS_FAULT, M64K_EXC_RESET,
                                M64K_FAULT_STAGE_FETCH, 32'd0, 32'd0);
                    ...
                    if (trace_pending_q) begin
                        ...
                        if (instruction_exception.valid) begin
                            if (instruction_exception.vector inside {
                                M64K_VECTOR_ACCESS_FAULT,
                                M64K_VECTOR_ADDRESS_ERROR}) begin
                                pending_exception_q <=
                            ...
                            pending_exception_q <= make_execute_exception(
                                M64K_VECTOR_PRIVILEGE, instruction_uop);
                            pending_exception_sr_q <= debug_sr;
                            pending_memory_action_q <=
                                MEMORY_ACTION_EXCEPTION_PUSH_PC;
                            pending_memory_address_q <= debug_ssp - 32'd4;
                            ...
                            pending_exception_q <= make_execute_exception(
                                M64K_VECTOR_ILLEGAL, instruction_uop);
                            pending_exception_sr_q <= debug_sr;
                            pending_memory_action_q <=
                                MEMORY_ACTION_EXCEPTION_PUSH_PC;
                            pending_memory_address_q <= debug_ssp - 32'd4;
                            ...
                            pending_exception_q <=
                                ...
                                M64K_VECTOR_CHK, instruction_uop);
                            trace_after_exception_q <= debug_sr[M64K_SR_T1];
                            ...
                            pending_memory_write_value_q <=
                                instruction_uop.sequential_pc;
                            state_q <= CORE_DATA_SEND;
                        end else if (instruction_uop.opcode == M64K_UOP_RESET) begin
                            ...
                            if (starts_memory_sequence) begin
                                ...
                                case (instruction_uop.opcode)
                                    ...
                                    M64K_UOP_MOVE,
                                    M64K_UOP_MOVE_ADDRESS: begin
                                        if (m64k_ea_is_memory(
                                    ...
                                    M64K_UOP_ADD_EXTEND,
                                    M64K_UOP_SUBTRACT_EXTEND: begin
                                        if (m64k_ea_is_memory(
                                                instruction_uop.source_ea)) begin
                                    ...
                                    M64K_UOP_JUMP_SUBROUTINE,
                                    M64K_UOP_BRANCH_SUBROUTINE: begin
                                        pending_memory_action_q <=
                                            MEMORY_ACTION_JSR_PUSH;
                                        pending_memory_address_q <=
                                    ...
                                    M64K_UOP_PUSH_EFFECTIVE_ADDRESS: begin
                                        pending_memory_action_q <=
                                            MEMORY_ACTION_PEA_PUSH;
                                        pending_memory_address_q <=
                                            packed_register_value(
                                    ...
                                    M64K_UOP_EXCEPTION_RETURN: begin
                                        pending_memory_action_q <=
                                            MEMORY_ACTION_RTE_LOAD_SR;
                                ...
                                if ((instruction_uop.opcode == M64K_UOP_BRANCH) &&
                                    branch_taken) begin
                                ...
                                if (instruction_uop.opcode == M64K_UOP_JUMP) begin
                                    redirect_target_q <= source_ea_address;
                                    redirect_pending_q <= 1'b1;
                                end

                                if (instruction_uop.opcode == M64K_UOP_STOP)
                                    state_q <= debug_sr[M64K_SR_T1] ?
                ...
                CORE_DATA_WAIT: begin
                    if (data_response_handshake) begin
                        ...
                        split_access_second_q <= 1'b0;
                        if (data_response_fault != M64K_FAULT_NONE) begin
                            sequence_program_q <= M64K_SEQUENCE_NONE;
                            sequence_step_q <= M64K_SEQUENCE_IDLE;
                            ...
                            state_q <= CORE_DIVIDE_WAIT;
                                     ...
                                     MEMORY_ACTION_SEQUENCE_READ) &&
                                    (sequence_step_q ==
                                     M64K_SEQUENCE_READ_SOURCE)) begin
                            ...
                            pending_exception_q <=
                                make_instruction_trap_exception(
                                M64K_VECTOR_CHK, pending_memory_uop_q);
