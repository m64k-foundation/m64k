module m64k_instruction_buffer #(
    ...
    input m64k_pkg::m64k_mem_fault_t input_fault,
    ...
    m64k_mem_fault_t fault_memory [0:DEPTH_WORDS-1];
    ...
    initial begin
        if (DEPTH_WORDS < 2)
            $fatal(1, "m64k_instruction_buffer DEPTH_WORDS must be at least two");
        if ((DEPTH_WORDS & (DEPTH_WORDS - 1)) != 0)
            $fatal(1, "m64k_instruction_buffer DEPTH_WORDS must be a power of two");
    end
        ...
        if (!rst_n) begin
            ...
            if (push) begin
                if (sequence_started_q && (input_pc != expected_input_pc_q))
                    $error("m64k_instruction_buffer non-sequential PC: expected %08x, got %08x",
