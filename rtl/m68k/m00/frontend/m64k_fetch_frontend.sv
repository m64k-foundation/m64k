module m64k_fetch_frontend #(
    ...
    output m64k_pkg::m64k_mem_fault_t word_fault,
    ...
    m64k_mem_fault_t slot0_fault_q;

    logic slot1_valid_q;
    logic [31:0] slot1_line_q;
    logic [127:0] slot1_data_q;
    m64k_mem_fault_t slot1_fault_q;

    m64k_mem_req_t memory_request;
    m64k_mem_fault_t response_fault;

    wire [31:0] current_line = m64k_line_base(fetch_pc_q);
    wire [3:0] current_lane = fetch_pc_q[3:0];
    wire slot0_is_current = slot0_valid_q && (slot0_line_q == current_line);
    wire buffered_fault = (slot0_valid_q && (slot0_fault_q != M64K_FAULT_NONE)) ||
                          (slot1_valid_q && (slot1_fault_q != M64K_FAULT_NONE));
    ...
    initial begin
        if (EPOCH_WIDTH < 4)
            $fatal(1, "m64k_fetch_frontend EPOCH_WIDTH must be at least four");
    end

    always_comb begin
        memory_request = '0;
        memory_request.command = M64K_MEM_READ;
        memory_request.size = M64K_SIZE_LINE;
        ...
        response_fault = response_ids_match ? mem.rsp.fault : M64K_FAULT_BUS;

        word_valid = slot0_is_current && !redirect_valid;
        word_pc = fetch_pc_q;
        word_fault = slot0_fault_q;
        word_data = 16'h0000;
        if (slot0_is_current && (slot0_fault_q == M64K_FAULT_NONE)) begin
            word_data = {
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            slot0_line_q <= '0;
            slot0_data_q <= '0;
            slot0_fault_q <= M64K_FAULT_NONE;
            slot1_valid_q <= 1'b0;
            slot1_line_q <= '0;
            slot1_data_q <= '0;
            slot1_fault_q <= M64K_FAULT_NONE;
            ...
            if (pop_word) begin
                ...
                if (slot0_fault_q != M64K_FAULT_NONE) begin
            ...
            if (response_handshake) begin
                if (!response_is_current) begin
                    perf_stale_responses <= perf_stale_responses + 1'b1;
                             ...
                             (slot0_fault_q == M64K_FAULT_NONE)) begin
                    ...
                    $error("m64k_fetch_frontend received a line with no queue space");
                end
            end

            if (redirect_valid) begin
                epoch_q <= epoch_q + 1'b1;
                next_line_q <= m64k_line_base(redirect_pc);
                fetch_pc_q <= redirect_pc;
                stream_supervisor_q <= supervisor;
                slot0_valid_q <= redirect_pc[0];
                slot0_line_q <= m64k_line_base(redirect_pc);
                slot0_data_q <= '0;
                slot0_fault_q <= redirect_pc[0] ?
                                 M64K_FAULT_ALIGNMENT : M64K_FAULT_NONE;
                slot1_valid_q <= 1'b0;
