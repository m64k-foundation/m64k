#!/usr/bin/env python3
"""Validate an MX68K ISA subset and generate its synthesizable lookup table."""
...
def render_leaf(entries: list[dict[str, object]], indent: str) -> list[str]:
    ...
    for index, item in enumerate(entries):
        ...
        lines.extend([
            ...
            f"{indent}    result.instruction_id = MX_INSN_{item['name']};",
            f"{indent}    result.format = MX_DECODE_{item['format']};",
            f"{indent}end",
        ])
    return lines
...
def render(entries: list[dict[str, object]], source: Path) -> str:
    enum_ids = ["        MX_INSN_UNKNOWN = 8'd0"]
    enum_ids.extend(
        f"        MX_INSN_{item['name']} = 8'd{item['id']}" for item in entries
    )
    used_formats = {str(item["format"]) for item in entries}
    formats = [name for name in FORMAT_ORDER if name in used_formats]
    enum_formats = ["        MX_DECODE_NONE = 6'd0"]
    enum_formats.extend(
        f"        MX_DECODE_{name} = 6'd{index}"
    ...
    return f"""// Generated by scripts/gen_mx68k_decode.py from isa/{source.name}.
