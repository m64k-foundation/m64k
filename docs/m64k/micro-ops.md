# M64K micro-operation architecture

## Decision

M64K uses a hybrid decoded-micro-operation design. M68k remains the only base
...
VECTOR    reserved M64K-V operation
...
future M64K-A operations use explicit fabric atomicity instead.
...
The first implemented slice lives in `isa/m64k_m00.json`. The generator
`scripts/gen_m64k_decode.py` validates identifiers, masks, values and opcode
overlap, then emits
`rtl/m68k/m00/decode/generated/m64k_m00_decode_table_pkg.sv`. Every MX test
first checks that the committed table exactly matches its JSON source.

`m64k_uop_pkg.sv` is the implemented decoder/backend contract. Besides the
