# M64K frontend and pipeline plan
...
The initial `m64k_fetch_frontend` allows one miss in flight, tags it with an
...
`m64k_decode_frontend` currently connects fetch, instruction buffering and the
...
scoreboard for iterative multiply/divide, LSU, FPU and future M64K-V operations.
...
may change without affecting M64K software compatibility.
