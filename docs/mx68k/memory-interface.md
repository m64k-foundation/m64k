# MX68K internal memory interface v0

The first implemented MX68K block is a board-neutral, decoupled memory
...
cannot pass older requests. An accepted `MX_MEM_ATOMIC` request is one
...
an explicit fault rather than silently degrading to read/write.
...
`mx68k_router_3` decodes three non-overlapping, prefix-masked regions. It allows
one routed transaction in flight, preserves IDs and backpressure, statically
rejects malformed/overlapping maps, and returns `MX_FAULT_ACCESS` for an
unmapped address.

`mx68k_fetch_frontend` is the first native master. It performs cache-line reads,
